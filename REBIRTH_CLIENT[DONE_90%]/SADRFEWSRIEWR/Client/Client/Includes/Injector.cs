    ...
    public class Injector : IDisposable
    {
        ...
        private class InjectedModule
        {
            ...
            private IntPtr FindExport(string func)
            {
                ...
                IntPtr pExportAddr = IntPtr.Zero;
                try
                {
        ...
        public void InjectLibrary(string libPath)
        {
            ...
            IntPtr pLibFullPathUnmanaged = Marshal.StringToHGlobalAnsi(fullPath); // unmanaged C-String pointer

            try
            {
                uint sizeUni = (uint)Encoding.Default.GetByteCount(fullPath) + 1;

                // Get Handle to Kernel32.dll and pointer to LoadLibraryW
                IntPtr hKernel32 = Imports.GetModuleHandle("Kernel32");
                if (hKernel32 == IntPtr.Zero)
                    throw new Win32Exception(Marshal.GetLastWin32Error());
                IntPtr hLoadLib = Imports.GetProcAddress(hKernel32, "LoadLibraryA");
                ...
                try
                {
                    Imports.WaitForSingleObject(hThread, 7000);
                }
                catch
                {
        ...
        public bool InjectLibraryW(string fullepath)
        {
            // (in?)sanity check, pretty sure this is never possible as the constructor will error - left over from how it previously was developed
            if (_process == null)
                throw new InvalidOperationException("This injector has no associated process and thus cannot inject a library");
            if (_handle == IntPtr.Zero)
                throw new InvalidOperationException("This injector does not have a valid handle to the associated process and thus cannot inject a library");
            int module1co = _process.Modules.Count;

            if (!File.Exists(fullepath))
                throw new FileNotFoundException(string.Format("Unable to find library {0} to inject into process {1}", fullepath, _process.ProcessName), fullepath);

            // convenience variables
            string fullPath = fullepath;
            string libName = Path.GetFileName(fullPath);

            foreach (ProcessModule mdlx in _process.Modules)
            {
                if (libName.ToLower() == Path.GetFileName(mdlx.FileName).ToLower())
                    return false;
            }

            string unipath = Encoding.Unicode.GetString(Encoding.Convert(Encoding.Default, Encoding.Unicode, Encoding.Default.GetBytes(fullPath)));

            // declare resources that need to be freed in finally
            IntPtr pLibRemote = IntPtr.Zero; // pointer to allocated memory of lib path string
            IntPtr hThread = IntPtr.Zero; // handle to thread from CreateRemoteThread
            IntPtr pLibFullPathUnmanaged = Marshal.StringToHGlobalUni(unipath); // unmanaged C-String pointer
            ...
            try
            {
                ...
                try
                {
                    Imports.WaitForSingleObject(hThread, 600);
                    _process.Refresh();
                }
                catch
                {
                ...
                foreach (ProcessModule mod in _process.Modules)
                {
                    if (mod.FileName.ToLower() == fullepath.ToLower())
                    {
                        modFound = mod;
                        break;
                    }
                ...
                if (modFound == null)
                    throw new Exception("Injected module could not be found within the target process! " + module1co.ToString() + " of " + _process.Modules.Count);

                injectedModules.Add(Path.GetFileName(fullepath), new InjectedModule(modFound));
            }
            catch
            {
        ...
        public void EjectLibraryXXX(ProcessModule mdl)
        {
            IntPtr baseaddrx = mdl.BaseAddress;
            ...
            try
            {
                ...
                hThread = Imports.CreateRemoteThread(_handle, IntPtr.Zero, 0, hFreeLib, baseaddrx, 0, IntPtr.Zero);
                if (hThread == IntPtr.Zero)
                    throw new Win32Exception(Marshal.GetLastWin32Error());
                if (Imports.WaitForSingleObject(hThread, (uint)ThreadWaitValue.Infinite) != (uint)ThreadWaitValue.Object0)
                    throw new Win32Exception(Marshal.GetLastWin32Error());
        ...
        public void EjectLibraryXXX(string libName)
        {
            IntPtr baseaddrx = IntPtr.Zero;

            foreach (ProcessModule mdl in _process.Modules)
            {
                if (Path.GetFileName(mdl.FileName).ToLower() == libName.ToLower())
                {
                    //   MessageBox.Show( mdl.BaseAddress.ToInt32().ToString() );
                    baseaddrx = mdl.BaseAddress;
                    break;
                }
            };
            if (baseaddrx == IntPtr.Zero)
                return;
        ...
        public void EjectLibraryMy(string libName)
        {
            string libSearchName = Path.GetFileName(libName);
            ...
            try
            {
                ...
                injectedModules.Remove(libSearchName);
            }
            finally
            {
                Imports.CloseHandle(hThread);
        ...
        public IntPtr CallExportAddr<T>(uint timeout, string libName, IntPtr FuncAddr, T data) where T : struct
        {
            IntPtr pData = IntPtr.Zero;
            try
            {
                ...
                pData = Marshal.AllocHGlobal(dataSize);
                CustomMarshal.StructureToPtr(data, pData, true);
                return CallExportInternalFuncAddr(timeout, libName, FuncAddr, pData, typeof(T), (uint)dataSize);
            }
            finally
            {
                Marshal.FreeHGlobal(pData);
        ...
        private IntPtr CallExportInternalFuncAddr(uint timeout, string libName, IntPtr fFunc, IntPtr data, Type dataType, uint dataSize)
        {
            string libSearchName = Path.GetFileName(libName);
            ...
            IntPtr pFunc = (IntPtr)(fFunc.ToInt32() + injectedModules[libSearchName].Module.BaseAddress.ToInt32());
            ...
            try
            {
                ...
                if (!(data == IntPtr.Zero || dataSize == 0 || dataType == null))
                {
                    ...
                    if (pDataRemote == IntPtr.Zero)
                        throw new Win32Exception(Marshal.GetLastWin32Error() + "1");
                    ...
                    if (!Imports.WriteProcessMemory(_handle, pDataRemote, data, dataSize, out bytesWritten))
                        throw new Win32Exception(Marshal.GetLastWin32Error() + "2");
                }

                hThread = Imports.CreateRemoteThread(_handle, IntPtr.Zero, 0, pFunc, pDataRemote, 0, IntPtr.Zero);
                if (hThread == IntPtr.Zero)
                    throw new Win32Exception(Marshal.GetLastWin32Error() + "3");
                ...
                if (!(singleObject == (uint)ThreadWaitValue.Object0 || singleObject == (uint)ThreadWaitValue.Timeout))
                    throw new Win32Exception(Marshal.GetLastWin32Error() + "4");
                DateTime dt2 = DateTime.Now;
                TimeSpan span = dt2 - dt1;
                uint latency = (uint)span.TotalMilliseconds;
                if (timeout - latency > 0)
                {
                    Thread.Sleep(Convert.ToInt32(timeout - latency));
                }


                IntPtr pRet;
                if (!Imports.GetExitCodeThread(hThread, out pRet))
                    throw new Win32Exception(Marshal.GetLastWin32Error() + "5");
        ...
        private IntPtr CallExportInternal(uint timeout, string libName, string funcName, IntPtr data, Type dataType, uint dataSize)
        {
            ...
            {
                ProcessModule modFound = null;
                string fullepath = null;
                foreach (ProcessModule mod in _process.Modules)
                {
                    if (Path.GetFileName(mod.FileName).ToLower() == libName.ToLower())
                    {
                        fullepath = mod.FileName;
                        modFound = mod;
                        break;
                    }
                }
