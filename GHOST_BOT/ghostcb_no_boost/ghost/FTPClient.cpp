...
//         * Return value of std:std::string->find must be checked against npos.
//      - Now runs in unicode.
//      - Streams removed.
...
namespace nsFTP
{
   ...
   public:
      CMakeString& operator<<(DWORD dwNum)
      {
         ...
         // std::string size +1 because tsprintf needs the size including the terminating NULL character;
...
/// @param[out] strResponse Response of the server as std::string.
...
/// Parses a response std::string and extracts the ip address and port information.
/// @param[in]  strResponse The response std::string of a FTP server which holds
...
/// @param[out] Size of the file specified in pathname.
/// @return see return values of CFTPClient::SimpleErrorCheck
int CFTPClient::CrcHash(const tstring& strPath, long& CrcHash) const
{
   CReply Reply;
   if( !SendCommand(CCommand::XCRC(), strPath, Reply) )
      return FTP_ERROR;
   CrcHash = CCnv::TStringToLong(Reply.Value().substr(4).c_str());
   return SimpleErrorCheck(Reply);
}
