...
void CConfig::Write(std::string file )
{
	if ( m_File.empty( ) )
		m_File = file;
	CONSOLE_Print( "[CONFIG] open file for write [" + m_File + "]" );
	ofstream out;
	out.open( m_File );
	if ( out.fail( ) )
	{
	...
	if ( !out.fail( ) )
	{
		CONSOLE_Print( "[CONFIG] start cfg write [" + m_File + "]" );
		for ( auto cfg_entry : m_CFG )
		{
			out << cfg_entry.first << " = " << cfg_entry.second << endl;
		}
	}
	out.close( );
}
...
void CConfig::Read( std::string file )
{
	...
	if ( m_File.find( ".cfg" ) == std::string::npos )
		m_File += ".cfg";
