...
{
	CONSOLE_Print( "[MYSQL trace] thread create..." );
	try
	{
		std::thread mThread( std::reference_wrapper<CBaseCallable>( /*ref(*/ *callable/* )*/ ) );
	}
	catch( ... )
	{
		CONSOLE_Print( "[MYSQL] error spawning thread on attempt #1 [ unknown ], pausing execution and trying again in 50ms" );
		MILLISLEEP( 50 );

		try
		{
			thread Thread(  ref( *callable ) );
		}
		catch( ... )
		{
			CONSOLE_Print( "[MYSQL] error spawning thread on attempt #2 [ unknown ], giving up" );
...
CCallableGameAdd *CGHostDBMySQL :: ThreadedGameAdd( std::string server, std::string map, std::string gamename, std::string ownername, uint32_t duration, uint32_t gamestate, std::string creatorname, std::string creatorserver , std::string statstype )
{
	void *Connection = GetIdleConnection( );
	...
	CCallableGameAdd *Callable = new CMySQLCallableGameAdd( server, map, gamename, ownername, duration, gamestate, creatorname, creatorserver, Connection, m_BotID, m_Server, m_Database, m_User, m_Password, m_Port , statstype );
	CreateThread( Callable );
	m_OutstandingCallables++;
	return Callable;
}
...
CCallableDotAGameAdd *CGHostDBMySQL :: ThreadedDotAGameAdd( uint32_t gameid, uint32_t winner, uint32_t min, uint32_t sec , uint32_t firstblood, std::string statstype )
{
	void *Connection = GetIdleConnection( );
...
CCallableDotAPlayerAdd *CGHostDBMySQL :: ThreadedDotAPlayerAdd( uint32_t gameid, uint32_t colour, uint32_t kills, uint32_t deaths, uint32_t creepkills, uint32_t creepdenies, uint32_t assists, uint32_t gold, uint32_t neutralkills, std::string item1, std::string item2, std::string item3, std::string item4, std::string item5, std::string item6, std::string hero, uint32_t newcolour, uint32_t towerkills, uint32_t raxkills, uint32_t courierkills,uint32_t herolevel )
{
	...
	CCallableDotAPlayerAdd *Callable = new CMySQLCallableDotAPlayerAdd( gameid, colour, kills, deaths, creepkills, creepdenies, assists, gold, neutralkills, item1, item2, item3, item4, item5, item6, hero, newcolour, towerkills, raxkills, courierkills, herolevel, Connection, m_BotID, m_Server, m_Database, m_User, m_Password, m_Port );
	CreateThread( Callable );
	m_OutstandingCallables++;
	return Callable;
}
...
CCallableW3MMDVarAdd *CGHostDBMySQL :: ThreadedW3MMDVarAdd( uint32_t gameid, map<VarP,std::string> var_strings )
{
	void *Connection = GetIdleConnection( );
...
vector<std::string> MySQLFetchRow( MYSQL_RES *res )
{
...
uint32_t MySQLAdminCount( void *conn, std::string *error, uint32_t botid, std::string server )
{
	...
	else
	{
		...
		if( Result )
		{
			vector<std::string> Row = MySQLFetchRow( Result );
...
vector<std::string> MySQLAdminList( void *conn, std::string *error, uint32_t botid, std::string server )
{
	std::string EscServer = MySQLEscapeString( conn, server );
	vector<std::string> AdminList;
...
vector<CDBBan *> MySQLBanList( void *conn, std::string *error, uint32_t botid, std::string server )
{
	std::string EscServer = MySQLEscapeString( conn, server );
...
CDBGamePlayerSummary *MySQLGamePlayerSummaryCheck( void *conn, std::string *error, uint32_t botid, std::string name )
{
	...
	std::string Query = "SELECT MIN(DATE(datetime)), MAX(DATE(datetime)), COUNT(*), MIN(loadingtime), AVG(loadingtime), MAX(loadingtime), MIN(`left`/duration)*100, AVG(`left`/duration)*100, MAX(`left`/duration)*100, MIN(duration), AVG(duration), MAX(duration) FROM gameplayers LEFT JOIN games ON games.id=gameid WHERE LOWER(name)='" + EscName + "'";
...
uint32_t MySQLDotAGameAdd( void *conn, std::string *error, uint32_t botid, uint32_t gameid, uint32_t winner, uint32_t min, uint32_t sec, uint32_t firstblood, std::string statstype )
{
	uint32_t RowID = 0;

	if ( statstype == "default" )
		return RowID;

	std::string Escstatstype = MySQLEscapeString( conn, statstype );
	std::string Query = "INSERT INTO dotagames ( botid, gameid, winner, min, sec, firstblood, statstype ) VALUES ( " + UTIL_ToString( botid ) + ", " + UTIL_ToString( gameid ) + ", " + UTIL_ToString( winner ) + ", " + UTIL_ToString( min ) + ", " + UTIL_ToString( sec ) + ", " + UTIL_ToString( firstblood ) + ", '" + Escstatstype + "' )";
			...
			if( Row.size( ) == 10 )
			{
				...
				if( TotalGames > 0 )
				{
					...
					else
					{
						...
						if( Result2 )
						{
							vector<std::string> Row2 = MySQLFetchRow( Result2 );
					...
					else
					{
						...
						if( Result3 )
						{
							vector<std::string> Row3 = MySQLFetchRow( Result3 );
...
bool MySQLW3MMDVarAdd( void *conn, std::string *error, uint32_t botid, uint32_t gameid, map<VarP,int32_t> var_ints )
{
	if( var_ints.empty( ) )
		return false;
...
bool MySQLW3MMDVarAdd( void *conn, std::string *error, uint32_t botid, uint32_t gameid, map<VarP,double> var_reals )
{
	if( var_reals.empty( ) )
		return false;
...
bool MySQLW3MMDVarAdd( void *conn, std::string *error, uint32_t botid, uint32_t gameid, map<VarP,std::string> var_strings )
{
	...
	for( map<VarP,std::string> :: iterator i = var_strings.begin( ); i != var_strings.end( ); i++ )
	{
		std::string EscVarName = MySQLEscapeString( conn, i->first.second );
...
void CMySQLCallableDotAGameAdd :: operator( )( )
{
	...
	if( m_Error.empty( ) )
		m_Result = MySQLDotAGameAdd( m_Connection, &m_Error, m_SQLBotID, m_GameID, m_Winner, m_Min, m_Sec , m_FirstBlood, m_statstype );
