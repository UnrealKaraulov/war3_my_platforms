...
CGame::CGame( CGHost *nGHost, CMap *nMap, CSaveGame *nSaveGame, uint16_t nHostPort, unsigned char nGameState, std::string nGameName, std::string nOwnerName, std::string nCreatorName, std::string nCreatorServer, std::string statstype ) : CBaseGame( nGHost, nMap, nSaveGame, nHostPort, nGameState, nGameName, nOwnerName, nCreatorName, nCreatorServer )
{
	m_DBBanLast = NULL;
...
CGame :: ~CGame( )
{
	if ( m_CallableGameAdd && m_CallableGameAdd->GetReady( ) )
	{
		if ( m_CallableGameAdd->GetResult( ) > 0 )
		{
			...
			if ( m_Stats )
				m_Stats->Save( m_GHost, m_GHost->m_DB, m_CallableGameAdd->GetResult( ), m_CallableGameAdd->GetStatsType( ) );
		}
		else
...
bool CGame::EventPlayerBotCommand( CGamePlayer *player, std::string command, std::string payload )
{
	...
	*********************/
	...
	//
	if ( Command == "latency" || Command == "lat" || Command == "l" )
	{
		CONSOLE_Print( "lat1" );
		if ( player->GetSpoofed( ) && ( IsCreator( User ) || !GetPlayerFromName( m_CreatorName, false ) ) )
		{
			CONSOLE_Print( "lat2" );
			if ( m_GHost->m_HideCommands )
				HideCommand = true;
...
{
	if ( !m_CallableGameAdd->GetReady( ) && m_CallableGameAdd->GetElapsed( ) > 5000 )
	{
		m_CallableGameAdd->SetReady( true );
		CONSOLE_Print( "[GAME: " + m_GameName + "] fatal error mysql" );
	}
...
void CGame::SaveGameData( )
{
	...
	m_CallableGameAdd = m_GHost->m_DB->ThreadedGameAdd( m_GHost->m_BNETs.size( ) == 1 ? m_GHost->m_BNETs[ 0 ]->GetServer( ) : std::string( ), m_DBGame->GetMap( ), m_GameName, m_OwnerName, m_GameTicks / 1000, m_GameState, m_CreatorName, m_CreatorServer, m_statstype );
}
