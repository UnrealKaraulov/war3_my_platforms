...
CCurses :: CCurses( int nTermWidth, int nTermHeight, bool nSplitView, int nListType )
{
	...
	m_Buffers[B_INPUT]->push_back( pair<std::string, int>("", 0) );
...
void CCurses :: AddTab( std::string nName, TabType nType, uint32_t nId, BufferType nBufferType, WindowType nWindowType )
{
	STabData data;
	data.name = nName;
	data.type = nType;
...
{
	std::string name = m_TabData[m_SelectedTab].name;
...
void CCurses :: CompileFriends( )
{
	...
	for( uint32_t i = 0; i < 10; ++i )
	{
		...
		{
			m_Buffers[B_FRIENDS]->push_back( pair<std::string, int>( "[" + m_RealmData[i].RealmAlias + "]", 1 ) );
			
			for( uint32_t j = 0; j < m_RealmData[i].Friends.size( ); ++j )
			{
				m_Buffers[B_FRIENDS]->push_back( pair<std::string, int>( " " + m_RealmData[i].Friends[j].first, m_RealmData[i].Friends[j].second ) );
			}

			m_Buffers[B_FRIENDS]->push_back( pair<std::string, int>( "\n", 1 ) );
		}
	}
}
...
void CCurses :: CompileClan( )
{
	...
	for( uint32_t i = 0; i < 10; ++i )
	{
		...
		{
			m_Buffers[B_CLAN]->push_back( pair<std::string, int>( "[" + m_RealmData[i].RealmAlias + "]", 1 ) );
			
			for( uint32_t j = 0; j < m_RealmData[i].Clan.size( ); ++j )
			{
				m_Buffers[B_CLAN]->push_back( pair<std::string, int>( " " + m_RealmData[i].Clan[j].first, m_RealmData[i].Clan[j].second ) );
			}

			m_Buffers[B_CLAN]->push_back( pair<std::string, int>( "\n", 1 ) );
		}
	}
}
...
void CCurses :: CompileBans( )
{
	...
	for( uint32_t i = 0; i < 10; ++i )
	{
		...
		{
			m_Buffers[B_BANS]->push_back( pair<std::string, int>( "[" + m_RealmData[i].RealmAlias + "]", 1 ) );
			
			for( uint32_t j = 0; j < m_RealmData[i].Bans.size( ); ++j )
			{
				m_Buffers[B_BANS]->push_back( pair<std::string, int>( " " + m_RealmData[i].Bans[j].first, m_RealmData[i].Bans[j].second ) );
			}

			m_Buffers[B_BANS]->push_back( pair<std::string, int>( "\n", 1 ) );
		}
	}
}
...
void CCurses :: CompileAdmins( )
{
	...
	for( uint32_t i = 0; i < 10; ++i )
	{
		...
		{
			m_Buffers[B_ADMINS]->push_back( pair<std::string, int>( "[" + m_RealmData[i].RealmAlias + "]", 1 ) );
			
			for( uint32_t j = 0; j < m_RealmData[i].Admins.size( ); ++j )
			{
				m_Buffers[B_ADMINS]->push_back( pair<std::string, int>( " " + m_RealmData[i].Admins[j].first, m_RealmData[i].Admins[j].second ) );
			}

			m_Buffers[B_ADMINS]->push_back( pair<std::string, int>( "\n", 1 ) );
		}
	}
}
...
void CCurses :: CompileGames( )
{
	...
	if( m_GHost->m_Games.empty( ) && m_GHost->m_CurrentGame )
		m_Buffers[B_GAMES]->push_back( pair<std::string, int>( "0. " + m_GHost->m_CurrentGame->GetDescription( ) + "\n", 0 ) );
	else
	{
		...
		{
			m_Buffers[B_GAMES]->push_back( pair<std::string, int>( UTIL_ToString( i + 1 ) + ". " + m_GHost->m_Games[i]->GetDescription( ) + "\n", 0 ) );
		}
	}
}
...
void CCurses :: SetAttribute( SWindowData &data, std::string message, int flag, BufferType type, bool on )
{
	...
	if ( data.IsWindowChanged )
	{
		...
		for( vector<STabData> :: iterator i = m_TabData.begin( ); i != m_TabData.end( ); i++ )
		{
			if( (*i).windowType == type )
			{
				...
				for( std::string :: iterator j = name.begin( ); j != name.end( ); j++ )
...
void CCurses :: Print( std::string message, uint32_t realmId, bool toMainBuffer )
{
	message = UTIL_UTF8ToLatin1( message );
	pair<std::string, int> temp = pair<std::string, int>( message, 0 );
	if ( toMainBuffer )
	{
...
bool CCurses :: Update( )
{
	...
	{	
		...
		if( m_Buffers[B_INPUT]->size( ) > 1 )
		{
			if( c == KEY_UP )
			{
				...
				if( m_SelectedInput != m_Buffers[B_INPUT]->size( ) - 1 )
				{
					...
					m_Buffers[B_INPUT]->push_back( pair<std::string, int>( m_InputBuffer, 0 ) );
				}
				return false;
			}
			else if( c == KEY_DOWN )
		...
		else if( c == 22 )
		{
			...
			{
				m_InputBuffer += std::string( clipboard, length );
		...
		{
			// copy
			std::string clipboard = UTIL_Latin1ToUTF8(m_InputBuffer);
...
uint32_t CCurses :: GetRealmId( std::string &realmAlias )
{
	// Get id of a realm.
	for (uint32_t i = 0; i < m_GHost->m_BNETs.size(); ++i)
...
uint32_t CCurses :: GetMessageFlag( std::string &message )
{
...
void CCurses :: ChangeChannel( std::string channel, uint32_t realmId )
{
...
void CCurses :: AddChannelUser( std::string name, uint32_t realmId, int flag )
{
	...
	m_RealmData[realmId].ChannelUsers.push_back( pair<std::string, int>( name, flag ) );
	m_WindowData[W_CHANNEL].IsWindowChanged = true;
	m_WindowData[W_CHANNEL].Scroll++;
}

void CCurses :: UpdateChannelUser( std::string name, uint32_t realmId, int flag )
{
	for( Buffer :: iterator i = m_RealmData[realmId].ChannelUsers.begin( ); i != m_RealmData[realmId].ChannelUsers.end( ); i++ )
	{
		if( (*i).first == name )
...
void CCurses :: RemoveChannelUser( std::string name, uint32_t realmId )
{
	for( Buffer :: iterator i = m_RealmData[realmId].ChannelUsers.begin( ); i != m_RealmData[realmId].ChannelUsers.end( ); i++ )
	{
		if( (*i).first == name )
