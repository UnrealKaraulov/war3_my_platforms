...
CIncomingJoinPlayer *CGameProtocol :: RECEIVE_W3GS_REQJOIN( BYTEARRAY data )
{
	...
	if( ValidateLength( data ) && data.size( ) >= 20 )
	{
		...
		if( !Name.empty( ) && data.size( ) >= Name.size( ) + 30 )
		{
			...
			return new CIncomingJoinPlayer( HostCounter, std::string( Name.begin( ), Name.end( ) ), InternalIP );
		}
	}
...
CIncomingJoinPlayer :: CIncomingJoinPlayer( uint32_t nHostCounter, std::string nName, BYTEARRAY &nInternalIP )
{
...
CIncomingChatPlayer :: CIncomingChatPlayer( unsigned char nFromPID, BYTEARRAY &nToPIDs, unsigned char nFlag, std::string nMessage )
{
	m_Type = CTH_MESSAGE;
	m_FromPID = nFromPID;
	m_ToPIDs = nToPIDs;
...
CIncomingChatPlayer :: CIncomingChatPlayer( unsigned char nFromPID, BYTEARRAY &nToPIDs, unsigned char nFlag, std::string nMessage, BYTEARRAY &nExtraFlags )
{
