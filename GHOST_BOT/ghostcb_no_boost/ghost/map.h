...
private:
	...
	std::string m_HCLValidModes;					// config value: valid hcl modes for map
	std::string m_MapGameNameWithMode;				// config value: gamename with random mode
	// @end
...
public:
	...
	void SetMapType( std::string maptype ) {  m_MapType = maptype; }
	...
	std::string GetMapHCLValidModes( )			{ return m_HCLValidModes; }
	std::string GetMapGameNameWithMode( )		{ return m_MapGameNameWithMode; }
	std::string GetMapGameNameWithRandomMode();
	// @end
