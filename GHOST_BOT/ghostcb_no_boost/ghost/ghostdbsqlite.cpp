...
CGHostDBSQLite :: CGHostDBSQLite( CConfig *CFG ) : CGHostDB( CFG )
{
	...
	if( Statement )
	{
		...
		if( RC == SQLITE_ROW )
		{
			vector<std::string> *Row = m_DB->GetRow( );
...
vector<std::string> CGHostDBSQLite :: AdminList( std::string server )
{
...
vector<CDBBan *> CGHostDBSQLite :: BanList( std::string server )
{
...
bool CGHostDBSQLite :: W3MMDVarAdd( uint32_t gameid, map<VarP,std::string> var_strings )
{
...
CCallableGameAdd *CGHostDBSQLite :: ThreadedGameAdd( std::string server, std::string map, std::string gamename, std::string ownername, uint32_t duration, uint32_t gamestate, std::string creatorname, std::string creatorserver, std::string statstype )
{
	CCallableGameAdd *Callable = new CCallableGameAdd( server, map, gamename, ownername, duration, gamestate, creatorname, creatorserver,statstype );
...
CCallableDotAGameAdd *CGHostDBSQLite :: ThreadedDotAGameAdd( uint32_t gameid, uint32_t winner, uint32_t min, uint32_t sec, uint32_t firstblood, std::string statstype )
{
	CCallableDotAGameAdd *Callable = new CCallableDotAGameAdd( gameid, winner, min, sec , firstblood, statstype);
...
{
	CCallableDotAPlayerAdd *Callable = new CCallableDotAPlayerAdd( gameid, colour, kills, deaths, creepkills, creepdenies, assists, gold, neutralkills, item1, item2, item3, item4, item5, item6, hero, newcolour, towerkills, raxkills, courierkills,herolevel );
...
CCallableW3MMDVarAdd *CGHostDBSQLite :: ThreadedW3MMDVarAdd( uint32_t gameid, map<VarP,std::string> var_strings )
{
