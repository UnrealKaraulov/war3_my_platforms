...
CGHost :: CGHost( CConfig *CFG )
{
	...
	CONSOLE_Print( "[GHOST] ExtractScripts" );
	ExtractScripts( );
	...
	CONSOLE_Print( "[GHOST] MapCFG.Read" );
	CConfig MapCFG;
		...
		if( !m_ExitingNice && m_Enabled && !m_CurrentGame && m_Games.size( ) < m_MaxGames && m_Games.size( ) < m_AutoHostMaximumGames )
		{
			if( m_AutoHostMap->GetValid( ) )
			{
				...
				*/
				...
				if( GameName.size( ) <= 31 )
				{
					CreateGame( m_AutoHostMap, GAME_PUBLIC, false, GameName, m_AutoHostOwner, m_AutoHostOwner, m_AutoHostServer, false,"default" );
	...
	if( SFileOpenArchive( PatchMPQFileName.c_str( ), 0, MPQ_OPEN_FORCE_MPQ_V1, &PatchMPQ ) )
	{
		...
		if( SFileOpenFileEx( PatchMPQ, "Scripts\\common.j", 0, &SubFile ) )
		{
			...
			if( FileLength > 0 && FileLength != 0xFFFFFFFF )
			{
				...
				{
					CONSOLE_Print( "[GHOST] extracting Scripts\\common.j from MPQ file to [" + m_Warcraft3Path + "common.j]" );
					UTIL_FileWrite( m_Warcraft3Path + "common.j", (unsigned char *)SubFileData, BytesRead );
				}
				else
		...
		if( SFileOpenFileEx( PatchMPQ, "Scripts\\blizzard.j", 0, &SubFile ) )
		{
			...
			if( FileLength > 0 && FileLength != 0xFFFFFFFF )
			{
				...
				if( SFileReadFile( SubFile, SubFileData, FileLength, &BytesRead ,0) )
				{
					CONSOLE_Print( "[GHOST] extracting Scripts\\blizzard.j from MPQ file to [" + m_Warcraft3Path + "blizzard.j]" );
					UTIL_FileWrite( m_Warcraft3Path + "blizzard.j", (unsigned char *)SubFileData, BytesRead );
				}
				else
...
void CGHost :: CreateGame( CMap *map, unsigned char gameState, bool saveGame, std::string gameName, std::string ownerName, std::string creatorName, std::string creatorServer, bool whisper , std::string statstype )
{
	if( !m_Enabled )
	{
		for( vector<CBNET *> :: iterator i = m_BNETs.begin( ); i != m_BNETs.end( ); i++ )
	...
	if( saveGame )
		m_CurrentGame = new CGame( this, map, m_SaveGame, m_HostPort, gameState, gameName, ownerName, creatorName, creatorServer ,statstype );
	else
		m_CurrentGame = new CGame( this, map, NULL, m_HostPort, gameState, gameName, ownerName, creatorName, creatorServer ,statstype );

	m_CurrentGame->m_FastCooldown = false;
