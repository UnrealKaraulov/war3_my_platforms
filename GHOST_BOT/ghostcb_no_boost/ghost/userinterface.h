...
typedef std::vector<std::pair<std::string, int> > Buffer;
...
private:
	...
	// Buffers
	std::vector<Buffer *> m_Buffers;		// pointers so updating is "automatic" and there is no useless copying

	// RealmData
	std::vector<SRealmData> m_RealmData;

	// Windows
	std::vector<SWindowData> m_WindowData;

	// Tabs
	std::vector<STabData> m_TabData;

	// Input
	std::string m_InputBuffer;	// current input
	...
	// Drawing
	void SetAttribute( SWindowData &data, std::string message, int flag, BufferType type, bool on );
	...
	// Tab
	void AddTab( std::string nName, TabType nType, uint32_t nId, BufferType nBufferType, WindowType nWindowType );
	...
	// Misc
	uint32_t GetRealmId( std::string &realmAlias );					// id from alias
	uint32_t GetNextRealm( uint32_t realmId );					// next realm id
	bool IsConnected( uint32_t realmId, bool entry );			// are we connected at all?

	uint32_t GetMessageFlag( std::string &message );					// returns message flag, e.g. "bnet" = 4 which makes it colored yellow
...
public:
	...
	void Print( std::string message, uint32_t realmId, bool toMainBuffer );	// prints messages to buffers
	bool Update( );														// updates user interface, returns true when quitting

	// Channel / Custom list
	void ChangeChannel( std::string channel, uint32_t realmId );
	void AddChannelUser( std::string name, uint32_t realmId, int flag );
	void UpdateChannelUser( std::string name, uint32_t realmId, int flag );
	void RemoveChannelUser( std::string name, uint32_t realmId );
