...
# endif
		...
		static std::thread update_stats_thread;
		static std::thread web_interface_thread;
		static std::thread updatemapsthread;
		bool needupdatemaps = false;

		static void web_interface_mainthread()
		{
			...
			svr.Get("/", [](const Request& req, Response& res)
				{
					res.set_content("<form action=\"/updatemaplist\" method=\"get\" target=\"_blank\">Password:<input type=\"text\" name=\"password\"><br><input type=\"submit\" value=\"Обновить список карт\"></form>", "text/html; charset=windows-1251");
				});

			svr.Get("/updatemaplist", [](const Request& req, Response& res)
				{
					if (req.get_param_value("password") == "1020304050")
					{
						try
						{
							...
							{
								std::getline(infile, updatemapip);
								std::cout << "Using " << updatemapip << " for update maps"<< std::endl;
							}
							updatemapsthread = std::thread{ UpadeMapList, updatemapip, 21, "ftpmaps","ftpmaps" ,"/",std::string("maphostcodes/"),std::string("mapcodescustom/") };
							updatemapsthread.join();
							 needupdatemaps = true;
							res.set_content("Maps updated!", "text/plain");
						}
						catch (...)
						{
			...
			svr.set_base_dir("./avatars", "/avatars");
			svr.set_base_dir("./docs","/docs");
			svr.listen("0.0.0.0", 999);
		}
...
prefs_set_verify_account_email(false);
				...
				if (now > prev_time)
				{
					...
					if (needupdatemaps)
					{
						...
						conn_send_bot_command("!updatemaplist " + updatemapip + " 21 ftpmaps ftpmaps /");
					}

					UpdatePlayerStatsTick();
					UpdatePlayersTick();
