...
namespace pvpgn
{
	...
	namespace bnetd
	{
		...
		extern std::string account_get_locktext( t_connection * c, t_account * account, bool with_author, bool for_mute )
		{
			...
			if ( with_author )
			{
				if ( char const * author = ( for_mute ) ? account_get_auth_muteby( account ) : account_get_auth_lockby( account ) )
				{
					...
					{
						msgtemp += localize( c, " by Root Admin", author );
					}
				}
			}
		...
		*/
		...
		//here
		extern std::string account_get_hwid( t_account * account )
		{
			const char * retval = account_get_strattr( account, "BNET\\acct\\hwid" );
		...
		extern int account_set_hwid( t_account * account, std::string lang )
		{
			return account_set_strattr( account, "BNET\\acct\\hwid", lang.c_str( ) );
		}


		extern bool account_map_is_firstgame( t_account * acc, std::string statstype )
		{
			return account_get_numattr( acc, ( "BNET\\acct\\st_" + statstype + "_" + "FirstGame" ).c_str( ) ) == 0;
		}

		extern void account_map_set_firstgame( t_account * acc, std::string statstype, int value )
		{
			account_set_numattr( acc, ( "BNET\\acct\\st_" + statstype + "_" + "FirstGame" ).c_str( ), value ? 0 : 1 );
		}

		extern unsigned int account_map_get_stats( t_account * acc, std::string statstype, std::string statscategory )
		{
			if ( account_map_get_bancount( acc, statstype ) > 0 && statscategory == "mmr" )
				return 0;

			if ( account_map_is_firstgame( acc, statstype )  && statscategory == "mmr" )
			{
				return 1000;
				//account_map_set_firstgame( acc, statstype, 0 );
				//account_map_set_stats( acc, statstype, statscategory, 1000 );
			}

			return account_get_numattr( acc, ( "BNET\\acct\\st_" + statstype + "_" + statscategory ).c_str( ) );
		}

		extern unsigned int account_map_set_stats( t_account * acc, std::string statstype, std::string statscategory, unsigned int  stats )
		{
			if ( account_map_get_bancount( acc, statstype ) > 0 )
				return 0;
			return account_set_numattr( acc, ( "BNET\\acct\\st_" + statstype + "_" + statscategory ).c_str( ), stats );
		}

		extern unsigned int account_map_add_stats( t_account * acc, std::string statstype, std::string statscategory, unsigned int  stats )
		{
			if ( account_map_get_bancount( acc, statstype ) > 0 )
				return 0;
			return account_map_set_stats( acc, statstype, statscategory,
				account_map_get_stats( acc, statstype, statscategory ) + stats );
		}


		extern int account_map_get_bancount( t_account * acc, std::string statstype )
		{
			return account_get_numattr( acc, ( "BNET\\acct\\st_" + statstype + "_bancount" ).c_str( ) );
		}

		extern int account_map_set_bancount( t_account * acc, std::string statstype, int count )
		{
			return account_set_numattr( acc, ( "BNET\\acct\\st_" + statstype + "_bancount" ).c_str( ), count );
		}

		extern std::time_t account_is_banned_stats( t_account * acc )
		{
			std::time_t bannedtime = account_map_get_stats( acc, "global", "bantime" );
			std::time_t now;
			std::time( &now );
			return now < bannedtime ? bannedtime - now : 0;
		}


		extern void account_print_map_stats( t_account * acc, std::string statstype )
		{
			if ( !acc )
				return;
			t_connection * c = account_get_conn( acc );
			if ( c && !statstype.empty( ) )
			{
				std::string lmapname = ToLower( statstype );

				message_send_text( c, message_type_error, c,
					localize( c, "Real forum username: {}", account_get_name_forum( acc ) ) );

				if ( lmapname == "default" )
				{
					std::string stats = localize( c, ":r::c:00FF8000Stats of {}:n:Player::r::c:FFFFFF00{}:r::c:00FF8000:n:Games:{},Leaves:{}:r:",
						"default maps",
						account_get_name( acc ),
						account_map_get_stats( acc, lmapname, "games" ),
						account_map_get_stats( acc, lmapname, "leaves" ) );

					message_send_text( c, message_type_error_custom, c,
						stats );
				}
				else if ( lmapname.find( "dota" ) == 0 )
				{
					std::string dotastats1 = localize( c, ":r::c:00FF8000Stats of {}:n:Player::r::c:FFFFFF00{}:r::c:00FF8000:n:Pts:{},Wins:{},Looses:{}",
						statstype.c_str( ),
						account_get_name( acc ),
						account_map_get_stats( acc, lmapname, "mmr" ),
						account_map_get_stats( acc, lmapname, "wins" ),
						account_map_get_stats( acc, lmapname, "loses" ) );
					std::string dotastats2 = localize( c, ":r::c:00FF8000Leaves:{}, Streak:{}, MinStreak:{}, MaxStreak:{}.",
						account_map_get_stats( acc, lmapname, "leaves" ),
						account_map_get_stats( acc, lmapname, "streak" ),
						account_map_get_stats( acc, lmapname, "minstreak" ),
						account_map_get_stats( acc, lmapname, "maxstreak" ) );
		...
		extern unsigned int account_get_superacc_num( t_account * acc )
		{
			return account_get_numattr( acc, "BNET\\acct\\superaccount" );
		}

		extern void account_set_superacc_num( t_account * acc, unsigned int num )
		{
			account_set_numattr( acc, "BNET\\acct\\superaccount", num );
		}
		...
		extern void account_add_newevent( t_account * acc, int id )
		{
			if ( !acc )
				return;
			account_set_numattr( acc, ( "BNET\\acct\\eventid_" + std::to_string( id ) ).c_str( ), account_get_numattr( acc, ( "BNET\\acct\\eventid_" + std::to_string( id ) ).c_str( ) ) + 1 );
		}

		extern void account_set_playmap( t_account * acc, const char * mapcode )
		{
			if ( !acc )
				return;
			account_set_strattr( acc, "BNET\\acct\\playingmap", mapcode );
		}
