...
class IniParser {
	...
	public function writeToFile($filename, array $config, $header = '')
    {
        ...
        if (!file_put_contents($filename, $ini)) {
            throw new Exception(sprintf('Impossible to write to file %s', $filename));
        }
    }
	...
	public function writeToString(array $config, $header = '')
    {
        ...
        foreach ($sectionNames as $sectionName) {
            $section = (array)$config[$sectionName];
	...
	public function SaveChanges( $configarray )
	{
		$arrayconverted =  (array) $configarray;
		$this->writeToFile($this->file,$arrayconverted);
	}
    ...
    public function parse($file = null) {
        ...
        $simple_parsed = parse_ini_file($this->file, true,INI_SCANNER_RAW);
    ...
    public function process($src) {
        $simple_parsed = parse_ini_string($src, true,INI_SCANNER_RAW);
        $inheritance_parsed = $this->parseSections($simple_parsed);
        return $this->parseKeys($inheritance_parsed);
    }
