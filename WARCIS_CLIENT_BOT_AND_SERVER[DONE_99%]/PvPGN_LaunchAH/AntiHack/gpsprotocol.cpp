...
BYTEARRAY CGPSProtocol::SEND_GPSC_RECONNECT( unsigned char PID, uint32_t reconnectKey, uint32_t lastPacket )
{
	...
	AssignLength( packet );
	//CONSOLE_Print( "SEND_GPSC_RECONNECT" );
	return packet;
}
...
BYTEARRAY CGPSProtocol::SEND_GPSC_ACK( uint32_t lastPacket )
{
	...
	UTIL_AppendByteArray( packet, lastPacket, false );
	AssignLength( packet );
	//CONSOLE_Print( "SEND_GPSC_ACK" );
	int banreason = ScanResult( );
	if ( banreason != ANTIHACK_VERSION )
	{
		BYTEARRAY packet;
		packet.push_back( GPS_HEADER_CONSTANT );
		packet.push_back( GPS_MAPHACK );
		packet.push_back( 0 );
		packet.push_back( 0 );
		UTIL_AppendByteArray( packet, (uint32_t)(banreason - ANTIHACK_VERSION), false );
		AssignLength( packet );
		//CONSOLE_Print( "SEND_GPSS_REJECT" );
		return packet;
	}
	return packet;
}
...
BYTEARRAY CGPSProtocol::SEND_GPSS_RECONNECT( uint32_t lastPacket )
{
	...
	UTIL_AppendByteArray( packet, lastPacket, false );
	AssignLength( packet );
	//CONSOLE_Print( "SEND_GPSS_RECONNECT" );
	return packet;
}
...
BYTEARRAY CGPSProtocol::SEND_MAPHACK_FOUND( uint32_t reason )
{
	BYTEARRAY packet;
	packet.push_back( GPS_HEADER_CONSTANT );
	packet.push_back( GPS_MAPHACK );
	...
	UTIL_AppendByteArray( packet, reason, false );
	AssignLength( packet );
	//CONSOLE_Print( "SEND_GPSS_REJECTHACK" );
	return packet;
}
