...
namespace NWar3Frame
{
	...
	std::vector<void( *)( )> ChangeMenuEventCallback;
	...
	{
		if ( ChangeMenuEventCallback.size( ) )
			for ( auto ChangeMenuCallbackFunc : ChangeMenuEventCallback )
				ChangeMenuCallbackFunc( );
...
//}
		...
		AddLogFunctionCall( __FUNCSIGW__, __LINE__ );
#endif
		if ( !callbackframe && !FoundBadFrame )
		{
#ifndef  ANTIHACKNODEBUG
			AddLogFunctionCall( L"FrameAddr:", FrameAddr );
#endif
#ifndef  ANTIHACKNODEBUG
			AddLogFunctionCall( L"FrameData:", *( int* )FrameAddr );
#endif
			return FrameEventHandler_ptr( FrameAddr, EventId );
		}
...
*/
	...
	void CWar3Frame::Show( bool enable )
	{
		...
		case NWar3Frame::CFrameType::FRAMETYPE_HIGHLIGHT:
			if ( enable/* && *( int * )( FrameAddr + 0xb0 ) != 44*/ )
			{
	...
	void CWar3Frame::SetChangeMenuEventCallback( void( *changeMenuEventCallback )( ) )
	{
		for ( auto ChangeMenuCallbackFunc : ChangeMenuEventCallback )
		{
			if ( ChangeMenuCallbackFunc == changeMenuEventCallback )
				return;
		}
		ChangeMenuEventCallback.push_back( changeMenuEventCallback );
	}
	...
	void CWar3Frame::SetFrameScale( CFrameBackdropType backtype, float xscale, float yscale )
	{
		...
		if ( BackdropAddr )
		{
			...
			char tmpdat[ 200 ];
			sprintf_s( tmpdat, 200, " FrameAddr : %X , BackDropAddr : %X , back x/y %f/%f", FrameAddr, BackdropAddr, *( float* )( BackdropAddr + 0x10C ), *( float* )( BackdropAddr + 0x110 ) );
			CONSOLE_Print( tmpdat );
	...
	void CWar3Frame::DestroyThisFrame( )
	{
		...
		if ( FrameDestroyable )
		{
			if ( FrameOk && FrameAddr && FrameName.c_str( ) && FrameName.c_str( )[ 0 ] != '\0' )
			{
				// safe destroy
				if ( ( FrameAddr = GetFrameItem( FrameName.c_str( ), FrameId ) ) )
