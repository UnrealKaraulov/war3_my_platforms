...
void CTCPSocket :: PutBytes(   const std::string & bytes )
{
	m_PutBytes.lock( );
	m_SendBuffer += bytes;
	m_PutBytes.unlock( );
...
void CTCPSocket :: DoRecv( fd_set *fd )
{
	...
	if( FD_ISSET( m_Socket, fd ) )
	{
		...
		if( c == SOCKET_ERROR && GetLastError( ) != WSAEWOULDBLOCK )
		{
			// receive error
...
void CTCPSocket :: DoSend( fd_set *send_fd )
{
	...
	if( FD_ISSET( m_Socket, send_fd ) )
	{
		...
		if( s == SOCKET_ERROR && GetLastError( ) != WSAEWOULDBLOCK )
		{
			// send error
...
void CTCPClient :: Connect(   const std::string & localaddress, const std::string & address, uint16_t port )
{
	...
	{
		if( GetLastError( ) != EINPROGRESS && GetLastError( ) != WSAEWOULDBLOCK )
		{
			// connect error
...
bool CUDPSocket :: SendTo( const std::string & address, uint16_t port, BYTEARRAY message )
{
	if( m_Socket == INVALID_SOCKET || m_HasError )
		return false;
...
bool CUDPServer :: Bind(  const std::string & address, uint16_t port )
{
	if( m_Socket == INVALID_SOCKET || m_HasError )
		return false;
