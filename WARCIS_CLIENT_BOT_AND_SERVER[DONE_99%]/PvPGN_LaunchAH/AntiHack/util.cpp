...
{
	// start searching the unsigned char array at position 'start' for the first null value
	...
	if ( start < b.size( ) )
	{
		...
		// no null value found, return the rest of the unsigned char array
...
BYTEARRAY UTIL_ExtractBytes( BYTEARRAY &b, unsigned int start )
{
	// start searching the unsigned char array at position 'start' for the first null value
	// if found, return the subarray from 'start' to the null value but not including the null value
...
{
	// consider the unsigned char array to contain a 2 character ASCII encoded hex value at b[start] and b[start + 1] e.g. "FF"
	// extract it as a single decoded unsigned char
...
BYTEARRAY UTIL_ExtractHexNumbers( const string & s )
{
...
bool UTIL_FileExists(   const std::string & file )
{
...
string UTIL_FileRead(  const std::string & file, uint32_t start, uint32_t length )
{
	ifstream IS;
...
string UTIL_FileRead(  const std::string & file )
{
	ifstream IS;
	IS.open( file.c_str( ), ios :: binary );
...
bool UTIL_FileWrite(  const std::string & file, unsigned char *data, uint32_t length )
{
	ofstream OS;
