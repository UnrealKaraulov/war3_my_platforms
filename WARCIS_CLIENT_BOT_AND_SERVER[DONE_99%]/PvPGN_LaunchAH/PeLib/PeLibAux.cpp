...
namespace PeLib
{
    const qword PELIB_IMAGE_ORDINAL_FLAGS<64>::IMAGE_ORDINAL_FLAG_V2 = 0x8000000000000000ULL;
	...
	{
		unsigned int oldpos = ( unsigned int )file.tellp();
	...
	**/
	unsigned int getFileType(const std::string strFilename)
	{
		...
		if (pef.readMzHeader() != NONO_ERROR) return PEFILE_UNKNOWN;
		if (pef.readPeHeader() != NONO_ERROR) return PEFILE_UNKNOWN;
	...
	**/
	PeFile* __stdcall openPeFile(const std::string& strFilename)
	{
