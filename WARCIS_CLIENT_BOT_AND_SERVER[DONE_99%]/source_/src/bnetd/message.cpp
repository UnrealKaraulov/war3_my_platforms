	...
	namespace bnetd
	{
		...
		static char const * message_type_get_str( t_message_type type )
		{
			...
			case message_type_talk:
				//here
			case message_type_talk_green:
			case message_type_talk_custom:
			case message_type_talk_yellow:
			case message_type_talk_blue:
				return "talk";
			...
			case message_type_error_custom:
				return "error";
...
#define MAX_INC 64
		...
		static int message_bnet_format( t_packet * packet, t_message_type type, t_connection * me, t_connection * dst, char const * text, unsigned int dstflags )
		{
			...
			if ( text )
			{
				temp = std::string( text );

				//here


				//eventlog( eventlog_level_error, __FUNCTION__, "In string:{}", text );
				bool skipreplace = false;
				t_account * senderacc = conn_get_account( me );
				if ( senderacc && IsHostBot( account_get_name( senderacc ) ) )
				{
					skipreplace = true;
				}

				if ( type == message_type_talk_custom || type == message_type_error_custom )
				{
					if ( type == message_type_talk_custom )
					{
						if ( !skipreplace )
							replaceAll( temp, ":n:", "-n-" );
					}
				...
				else
				{
					if ( !skipreplace )
					{
						...
						if ( strstr( temp.c_str( ), ":c:" )
							|| strstr( temp.c_str( ), ":r:" )
							|| strstr( temp.c_str( ), ":n:" ) )
						{
							eventlog( eventlog_level_error, __FUNCTION__, "got error in replaceAll" );
						}
					}
				}


				//eventlog( eventlog_level_error, __FUNCTION__, "Out string 1:{}", temp.c_str() );


				if ( type == message_type_talk_blue )
				{
					temp = ":c:FF0000FF" + temp + ":r:";
				}
				else if ( type == message_type_talk_green )
				{
					temp = ":c:FF00FF00" + temp + ":r:";
				}
				else if ( type == message_type_talk_yellow )
				{
					temp = ":c:FFFFFF00" + temp + ":r:";
				}
				...
				if ( temp.length( ) > MAX_MESSAGE_LEN ) {
					...
					temp = std::string( temp.begin( ), temp.begin( ) + MAX_MESSAGE_LEN - 1);
				}
			...
			//eventlog( eventlog_level_error, __FUNCTION__, "Out string 2:{}", text );


			//here
		/*	if ( type == message_type_talk_blue || type == message_type_talk_green ||
				type == message_type_talk_custom || type == message_type_talk_yellow )
			{
				type = message_type_talk;
			}*/
		...
		extern t_message * message_create( t_message_type type, t_connection * src, const char * text )
		{
			t_message * message;
			...
			//if ( type == message_type_talk_green )
			//{
			//	text = ("[c]FF00FF00" + text + ":r:");
			//}
			//if ( type == message_type_talk_blue )
			//{
			//	text = ("[c]FF0000FF" + text + ":r:");
			//}
			//if ( type == message_type_talk_yellow )
			//{
			//	text = ("[c]FFFFFF00" + text + ":r:");
			//}
