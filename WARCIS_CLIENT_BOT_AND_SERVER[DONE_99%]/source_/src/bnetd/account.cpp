	...
	namespace bnetd
	{
		...
		static t_account * account_create(char const * username, char const * passhash1)
		{
			...
			{ 
				...
				/*
				account_map_set_stats( account, "dota", "mmr", 1000 );
				account_map_set_stats( account, "dota_omg", "mmr", 1000 );
				account_map_set_stats( account, "dota_imba", "mmr", 1000 );

				account_set_strattr( account, "BNET\\acct\\statsdota_str1", "MMR:{1} WINRATE:{2} " );
				account_set_strattr( account, "BNET\\acct\\statsdota_str2", "no_stats" );
				account_set_strattr( account, "BNET\\acct\\statsdota_str3", "no_stats" );
*/
		...
		extern bool account_set_name_real( t_account * account, const char * username )
		{
			if ( !account )
				return false;
			if ( account_set_strattr( account, "BNET\\acct\\username", username ) < 0 ) {
		...
		//HERE
		extern char const * account_get_name_forum( t_account * account )
		{
			char const * temp;

			if ( !account )
			{
				eventlog( eventlog_level_error, __FUNCTION__, "got NULL account (from )" );
				return ""; /* FIXME: places assume this can't fail */
			}
			if ( !account )
				return "";
			temp = account_get_strattr( account, "BNET\\acct\\username_forum" );
			return temp ? temp : "";
		}

		extern bool account_set_name_forum( t_account * account, const char * username )
		{
			if ( !account )
				return false;
			if ( account_set_strattr( account, "BNET\\acct\\username_forum", username ) < 0 ) {
				eventlog( eventlog_level_error, __FUNCTION__, "could not set username_forum" );
				return false;
			}
			return true;
		}
