...
namespace pvpgn
{
	...
	namespace bnetd
	{
		...
		extern t_game * game_create( char const * name, char const * pass, char const * info, t_game_type type, int startver, t_clienttag clienttag, unsigned long gameversion )
		{
			...
			game->visibledname = ( char * )xmalloc( 64 );
			std::snprintf( game->visibledname, 64, "%.24s(0/0)", name );
			...
			//here
			game->statstype = ( char* )xmalloc( 100 );
			game->statstype[0] = '\0';
			game->intop = 0;
			game->minpts = 0;
			game->maxpts = 0;
			game->IsRatingGame = false;
			...
			eventlog( eventlog_level_info, __FUNCTION__, "game \"{}\"(or:{}) (pass \"{}\") type {}({}) startver {} created", name, game->visibledname, pass, ( unsigned short )type, game_type_get_str( game->type ), startver );
		...
		static void game_destroy( t_game * game )
		{
			...
			//here
			eventlog( eventlog_level_error, __FUNCTION__, "remove game from list" );
			RemoveHostedGameFromConnections( game );
			eventlog( eventlog_level_error, __FUNCTION__, "remove game ok" );
			...
			for ( i = 0; i < game->count; i++ )
			{
				eventlog( eventlog_level_debug, __FUNCTION__, "2.1" );
				...
				eventlog( eventlog_level_debug, __FUNCTION__, "2.3" );
				if (game->connections[i])
				{
			...
			eventlog( eventlog_level_debug, __FUNCTION__, "2" );
			if ( game->realmname )
			...
			if ( game->statstype )
				xfree( ( void * )game->statstype );
			game->IsRatingGame = false;

			eventlog( eventlog_level_debug, __FUNCTION__, "3" );
			xfree( ( void * )game->info ); /* avoid warning */
			xfree( ( void * )game->pass ); /* avoid warning */
			if ( game->name ) xfree( ( void * )game->name ); /* avoid warning */
			if ( game->visibledname ) xfree( ( void * )game->visibledname ); /* avoid warning */
		...
		extern char const * game_get_name( t_game const * _game )
		{
		...
		extern char const * game_get_visibledname( t_game const * game )
		{
			...
			std::snprintf( game->visibledname, 64, "%.24s(%i/%i)", game->name, game->ref - 1, game->maxplayers );
			return game->visibledname;
		}
		...
		extern void game_set_status( t_game * game, t_game_status status )
		{
			...
			{
				game->start_time = now;
				RemoveHostedGameFromConnections( game, false );
			}
			game->status = status;

			//eventlog( eventlog_level_error, "game_set_status", "Game new status {}", game_status_get_str( status ) );
		...
		extern int game_add_player( t_game * game, char const * pass, int startver, t_connection * c )
		{
			...
			if ( game->connections && ( game->count > 0 ) )
			{
				for ( i = 0; i < game->count; i++ )
				{
					...
					{
						if ( game_get_forstats( game ) )
						{
							conn_send_gamestats_to_player( game->connections[ i ], c, game_get_statstype( game ) );
							conn_send_gamestats_to_player( c, game->connections[ i ], game_get_statstype( game ) );
						}
					}
				}
			...
			if ( ( i == game->count ) || ( game->count == 0 ) )
			{
				...
				if ( acc )
				{
					if ( account_get_superacc_num( acc ) & SUPERACCOUNT_TOPGAME )
					{
						game->intop = 1;
					}
				}
			...
			conn_send_ah_followstate( c, 0, 0 );

			// if owner is bot - send spoofcheck command
			if ( game->owner )
			{
				t_account * owneracc = conn_get_account( game->owner );
				if ( owneracc && IsHostBot( account_get_name( owneracc ) ) )
				{
					message_send_text( game->owner, message_type_whisper, c, "s" );
				}
			}

			if ( game->minpts != 0 && game->maxpts != 0 )
			{
				game_print_dotaptslimit( game, c );
			}
		...
		extern t_game * gamelist_find_game( char const * name, t_clienttag ctag, t_game_type type )
		{
			...
			t_game *game;
			char tmpName[ 64 ];
			std::snprintf( tmpName,64, "%s", name );
			...
			elist_for_each( curr, &gamelist_head )
			{
				...
				if ( ( type == game_type_all || game->type == type )
					&& ctag == game->clienttag
					&& game->name
					&& !strcasecmp( tmpName, game->name ) ) return game;
			}

			if ( namelen > 0 && tmpName[ namelen - 1 ] == ')' )
			{
		...
		extern t_game * gamelist_find_game_available( char const * name, t_clienttag ctag, t_game_type type )
		{
			...
			elist_for_each( curr, &gamelist_head )
			{
				...
				if ( ( type == game_type_all || game->type == type ) && ( ctag == game->clienttag ) && ( game->name )
					&& ( !strcasecmp( tmpName, game->name ) ) && ( game->status != game_status_started ) &&
					( game->status != game_status_done ) )
				{
					return game;
				}
		...
		extern void gamelist_traverse( t_connection * c, t_glist_func cb, void *data, t_gamelist_source_type gamelist_source )
		{
			t_elist *curr;
			...
			elist_for_each( curr, &gamelist_head )
			{
				t_game * currentgame = elist_entry( curr, t_game, glist_link );
				if ( currentgame && currentgame->intop == 2 && game_check_access_for_player( currentgame, c ) )
				{
					if ( cb( elist_entry( curr, t_game, glist_link ), data ) < 0 ) break;
				}
				else if ( !currentgame )
				{
					break;
			...
			{
				t_game * currentgame = elist_entry( curr, t_game, glist_link );
				if ( currentgame && currentgame->intop == 1 && game_check_access_for_player( currentgame, c ) )
				{
					if ( cb( elist_entry( curr, t_game, glist_link ), data ) < 0 ) break;
				}
				else if ( !currentgame )
			...
			{
				t_game * currentgame = elist_entry( curr, t_game, glist_link );
				if ( currentgame && currentgame->intop == 0 && game_check_access_for_player( currentgame, c ) )
				{
					if ( cb( elist_entry( curr, t_game, glist_link ), data ) < 0 ) break;
				}
				else if ( !currentgame )
		...
		extern void game_set_forstats( t_game * game, bool stats )
		{
			if ( !game ) {
				eventlog( eventlog_level_error, __FUNCTION__, "got NULL game" );
				return;
			...
			game->IsRatingGame = stats;
		}
		extern bool game_get_forstats( t_game * game )
		{
			if ( !game ) {
				eventlog( eventlog_level_error, __FUNCTION__, "got NULL game" );
				return false;
			...
			return game->IsRatingGame;
		}

		//here
		extern void game_set_statstype( t_game * game, std::string statstype )
		{
			if ( !game ) {
				eventlog( eventlog_level_error, __FUNCTION__, "got NULL game" );
				return;
			...
			std::snprintf( game->statstype, 100, "%s", statstype.c_str( ) );

			if ( game->connections && game->count > 0 )
			{
				for ( int x = 0; x < game->count; x++ )
				{
					if ( game->connections[ x ] != NULL )
					{
						for ( int y = 0; y < game->count; y++ )
						{
							if ( game->connections[ y ] != NULL )
							{
								conn_send_gamestats_to_player( game->connections[ x ], game->connections[ y ], statstype );
								//	conn_send_gamestats_to_player( game->connections[ y ], game->connections[ x ], statstype );
							}
						}
					}
				}
		...
		extern void game_print_dotaptslimit( t_game * game, t_connection * c )
		{
			if ( !game ) {
				eventlog( eventlog_level_error, __FUNCTION__, "got NULL game" );
				return;
			...
			message_send_text( c, message_type_info, c,
				localize( c, "Game points limit: ({})-({}) mmr.", game->minpts, game->maxpts ) );
		}

		extern const char * game_get_statstype( t_game * game )
		{
			if ( !game ) {
				eventlog( eventlog_level_error, __FUNCTION__, "got NULL game" );
				return "";
			...
			if ( game->statstype &&  game->statstype[ 0 ] != '\0' )
				return game->statstype;

			return "";
		}

		extern void game_set_top( t_game * game, int intop )
		{
			if ( !game ) {
				eventlog( eventlog_level_error, __FUNCTION__, "got NULL game" );
				return;
		...
		// ѕровер€ет доступна ли игра игроку (по mmr т.е очкам)
		extern bool game_check_access_for_player( t_game * game, t_connection * c )
		{
			if ( !game || !c )
				return false;
			...
			if ( game->statstype && game->statstype[ 0 ] != '\0' )
			{
				...
				if ( acc )
				{
					int mmr = account_map_get_stats( acc, game->statstype, "mmr" );
					if ( game->maxpts > 0 && mmr > game->maxpts )
					{
						return false;
					}

					if ( game->minpts > 0 && mmr < game->minpts )
					{
						return false;
					}
				}
		...
		extern void game_set_dotaptslimit( t_game * game, int minpts, int maxpts )
		{
			if ( !game )
				return;
			// дл€ экономии 1 переменной добавлю пару лишних строк кода :D :D :D
			if ( minpts == 0 )
				game->minpts = minpts + 1;
			else
				game->minpts = minpts;


			if ( maxpts == 0 )
				game->maxpts = maxpts + 1;
			else
				game->maxpts = maxpts;
		...
		extern void game_set_count( t_game  * game, unsigned int count )
		{
			if ( !game )
			{
				eventlog( eventlog_level_error, __FUNCTION__, "got NULL game" );
		...
		extern void game_send_message( t_game * game, std::string message )
		{
			if ( game->connections && game->count > 0 )
			{
				for ( int x = 0; x < game->count; x++ )
				{
					if ( game->connections[ x ] != NULL )
					{
						message_send_text( game->connections[ x ], message_type_info, game->connections[ x ], message );
					}
				}
			}
		}
		...
		extern t_game * game_search_for_player( int mmr, std::string statstype )
		{
			t_elist *curr;
			t_game *game;
			elist_for_each( curr, &gamelist_head )
			{
				...
				if ( game )
				{
					if ( game->minpts >= mmr || mmr < game->maxpts )
					{
						if ( game->status == game_status_open && game->statstype && game->statstype[ 0 ] != '\0'
							&& stricmp(game->statstype, statstype.c_str()) == 0 )
							return game;
					}
				}
			}
