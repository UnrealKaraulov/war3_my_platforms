			...
			svr.Get("/updatemaplist", [](const Request& req, Response& res)
				{
					if (req.get_param_value("password") == "1020304050")
					{
						...
						{
							//84.201.151.100
							updatemapsthread = std::thread{ UpadeMapList, "84.201.151.100", 21, "ftpmaps","ftpmaps" ,"/",std::string("maphostcodes/"),std::string("mapcodescustom/") };
	...
	*/
				...
				if (now > prev_time)
				{
					...
					if (needupdatemaps)
					{
						...
						conn_send_bot_command("!updatemaplist 84.201.151.100 21 ftpmaps ftpmaps /");
					}
