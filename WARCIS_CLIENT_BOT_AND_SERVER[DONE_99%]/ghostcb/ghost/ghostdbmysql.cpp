...
CCallableGameAdd *CGHostDBMySQL :: ThreadedGameAdd( string server, string map, string gamename, string ownername, uint32_t duration, uint32_t gamestate, string creatorname, string creatorserver , string statstype )
{
	void *Connection = GetIdleConnection( );
...
CCallableDotAGameAdd *CGHostDBMySQL :: ThreadedDotAGameAdd( uint32_t gameid, uint32_t winner, uint32_t min, uint32_t sec , uint32_t firstblood, string statstype )
{
	void *Connection = GetIdleConnection( );
...
uint32_t MySQLDotAGameAdd( void *conn, string *error, uint32_t botid, uint32_t gameid, uint32_t winner, uint32_t min, uint32_t sec, uint32_t firstblood, string statstype )
{
	uint32_t RowID = 0;
	...
	string Escstatstype = MySQLEscapeString( conn, statstype );
	string Query = "INSERT INTO dotagames ( botid, gameid, winner, min, sec, firstblood, statstype ) VALUES ( " + UTIL_ToString( botid ) + ", " + UTIL_ToString( gameid ) + ", " + UTIL_ToString( winner ) + ", " + UTIL_ToString( min ) + ", " + UTIL_ToString( sec ) + ", " + UTIL_ToString( firstblood ) + ", '" + Escstatstype + "' )";
