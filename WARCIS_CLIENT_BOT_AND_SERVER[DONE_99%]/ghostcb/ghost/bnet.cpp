	...
	string FtpStartDir, string mappath, string mapcfgpath )
{
	...
	if ( ftpClient.Login( logonInfo ) )
	{
			...
			// get directory listing
		{
			...
			vector<string> RemoteFileList;
			// iterate listing
			for ( nsFTP::TFTPFileStatusShPtrVec::iterator it = list.begin( );
				it != list.end( ); ++it )
			{
				RemoteFileList.push_back( ( *it )->Name( ) );
				path RemoteFilePath( ( *it )->Path( ) + ( *it )->Name( ) );
				...
				if ( RemoveFileExt == ".w3x" || RemoveFileExt == ".w3m" )
				{
					...
					path FilePath( MapPath.string( ) + ( *it )->Name( ) );
					...
					if ( needReplaceFile )
					{
						ftpClient.DownloadFile( RemoteFilePath.string( ), FilePath.string( ) );
...

		{
				...
				it != list.end( ); ++it )
			{
				...
				bool needReplaceFile = true;
				path FilePath( MapCfgPath.string( ) + ( *it )->Name( ) );
				auto filedata = readFile( FilePath.string( ) + ".time" );
				time_t localfiletime = 0;
				time_t remotefiletime = ( *it )->MTime( );
			...
			|| MessageLower.find( "-l" ) == 0 ) )
		{
			...
			if ( IsAdmin( User ) || IsRootAdmin( User ) )
			{
				...
				******************/
				...
				if ( Command == "updatemaplist" && !Payload.empty( ) )
				{
					string FtpIp = string( );
					int FtpPort;
				...
				if ( Command == "hold" && !Payload.empty( ) && m_GHost->m_CurrentGame )
				{
					...
					// Команда временно отключена нужно немного продумать ее
					/*stringstream SS;
					SS << Payload;
