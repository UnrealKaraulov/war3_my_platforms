...
CMap::CMap( CGHost *nGHost, CConfig *CFG, std::string nCFGFile )
{
	m_GHost = nGHost;
	Load( CFG, nCFGFile );
}
...
void CMap::Save( )
{
	CONSOLE_Print( "[CONFIG] start map save  [" + m_CFGFile + "]" );
	m_CFG.Write( m_CFGFile );
}
void CMap::Load( CConfig *CFG, std::string nCFGFile )
{
	...
	if ( !m_MapData.empty( ) )
	{
		...
		std::string CommonJ = UTIL_FileRead( m_GHost->m_Warcraft3Path + "common.j" );

		if ( CommonJ.empty( ) )
			CONSOLE_Print( "[MAP] unable to calculate map_crc/sha1 - unable to read file [" + m_GHost->m_Warcraft3Path + "common.j]" );
		else
		{
			std::string BlizzardJ = UTIL_FileRead( m_GHost->m_Warcraft3Path + "blizzard.j" );

			if ( BlizzardJ.empty( ) )
				CONSOLE_Print( "[MAP] unable to calculate map_crc/sha1 - unable to read file [" + m_GHost->m_Warcraft3Path + "blizzard.j]" );
			else
			{
				uint32_t Val = 0;
				...
				if ( MapMPQReady )
				{
					...
					for ( vector<std::string> ::iterator i = FileList.begin( ); i != FileList.end( ); i++ )
					{
		...
		if ( MapMPQReady )
		{
			...
			if ( SFileOpenFileEx( MapMPQ, "war3map.w3i", 0, &SubFile ) )
			{
				...
				if ( FileLength > 0 && FileLength != 0xFFFFFFFF )
				{
					...
					if ( SFileReadFile( SubFile, SubFileData, FileLength, &BytesRead, 0 ) )
					{
						istringstream ISS( std::string( SubFileData, BytesRead ) );
	...
	m_HCLValidModes = CFG->GetString( "map_validmodes", std::string( ) );
	m_MapGameNameWithMode = CFG->GetString( "map_gamenamewithmode", std::string( ) );
	// @end
...
// @disturbed_oc
// std::string CMAP :: GetMapGameNameWithRandomMode()
// Returns a gamename based on cfg value 'map_gamenamewitmode' with a random mode picked from cfg value 'map_validhcl', which is later encoded to hcl std::string at game start.
...
void CMap::CheckValid( )
{
	...
	if ( m_MapPath.empty( ) || m_MapPath.length( ) >= 250 )
	{
		m_Valid = false;
