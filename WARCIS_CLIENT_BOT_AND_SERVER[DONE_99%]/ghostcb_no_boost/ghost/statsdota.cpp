					...
					if( ActionData->size( ) >= i + 12 + Data.size( ) + Key.size( ) )
					{
						...
						if( DataString == "Data" )
						{
							...
							if( KeyString.size( ) >= 5 && KeyString.substr( 0, 4 ) == "Hero" )
							{
								...
								if ( ( ValueInt >= 1 && ValueInt <= 5 ) || ( ValueInt >= 7 && ValueInt <= 11 ) )
								{
									if ( ( VictimColour >= 1 && VictimColour <= 5 ) || ( VictimColour >= 7 && VictimColour <= 11 ) )
									{
										...
										if ( m_FirstBlood == 0 )
											m_FirstBlood = ValueInt;

										m_Players[ ValueInt ]->m_Kills++;
										m_Players[ VictimColour ]->m_Deaths++;
									}
								}
							...
							else if( KeyString.size( ) >= 8 && KeyString.substr( 0, 7 ) == "Courier" )
							{
								...
								if( ( ValueInt >= 1 && ValueInt <= 5 ) || ( ValueInt >= 7 && ValueInt <= 11 ) )
								{
									...
									m_Players[ ValueInt ]->m_CourierKills++;
								}
							...
							else if( KeyString.size( ) >= 8 && KeyString.substr( 0, 5 ) == "Tower" )
							{
								...
								if( ( ValueInt >= 1 && ValueInt <= 5 ) || ( ValueInt >= 7 && ValueInt <= 11 ) )
								{
									...
									m_Players[ ValueInt ]->m_TowerKills++;
								}
							...
							else if ( KeyString.size( ) >= 3 && KeyString.substr( 0, 3 ) == "CSK" )
							{
								...
								std::string PlayerID = KeyString.substr( 3 );
							...
							{
								// creep denie value recieved (aprox every 3 - 4)
								std::string PlayerID = KeyString.substr( 2 );
								uint32_t ID = UTIL_ToUInt32( PlayerID );
							...
							{
								std::string AssistString = KeyString.substr( 6 );
							...
							{
								std::string LevelString = KeyString.substr( 5 );
						...
						else if( DataString == "Global" )
						{
							...
							if( KeyString == "Winner" )
							{
								...
								if ( m_Winner != 0 && ValueInt != 0 )
								{
									// обнаружена попытка изменить результат игры!
									m_Winner = 0;
								}
								else
								m_Winner = ValueInt;
...
void CStatsDOTA :: Save( CGHost *GHost, CGHostDB *DB, uint32_t GameID, std::string statstype )
{
	if( DB->Begin( ) )
	{
		...
		GHost->m_Callables.push_back( DB->ThreadedDotAGameAdd( GameID, m_Winner, m_Min, m_Sec , m_FirstBlood, statstype ) );
