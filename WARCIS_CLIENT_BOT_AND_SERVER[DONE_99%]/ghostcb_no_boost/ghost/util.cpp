...
std::vector<std::string> get_file_list( const fs::path & dirpath )
{
	std::vector<std::string> m_file_list;
	if ( !dirpath.empty( ) )
	{
		fs::path apk_path( dirpath );
...
bool replaceAll( std::string & str, std::string from, std::string to ) {
	if ( from.empty( ) )
		return false;
...
std::string UTIL_Latin1ToUTF8( std::string &s )
{
	std::string result;
	int c;
...
std::string UTIL_UTF8ToLatin1( std::string & s )
{
	std::string temp, result = s;
...
vector<std::string> UTIL_Tokenize( std::string s, char delim )
{
...
std::vector<std::string> split_command( std::string text, int args_count )
{
