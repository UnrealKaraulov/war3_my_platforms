...
CStatsW3MMD :: CStatsW3MMD( CBaseGame *nGame, std::string nCategory ) : CStats( nGame )
{
				...
				if( ActionData->size( ) >= i + 11 + MissionKey.size( ) )
				{
					...
					if( ActionData->size( ) >= i + 15 + MissionKey.size( ) + Key.size( ) )
					{
						...
						if( MissionKeyString.size( ) > 4 && MissionKeyString.substr( 0, 4 ) == "val:" )
						{
							...
							vector<std::string> Tokens = TokenizeKey( KeyString );
							...
							if( !Tokens.empty( ) )
							{
								...
								else if( Tokens[0] == "DefEvent" && Tokens.size( ) >= 4 )
								{
									...
									else
									{
										...
										if( Tokens.size( ) == Arguments + 4 )
											m_DefEvents[Tokens[1]] = vector<std::string>( Tokens.begin( ) + 3, Tokens.end( ) );
									}
								}
								else if( Tokens[0] == "Event" && Tokens.size( ) >= 2 )
								{
									...
									{
										vector<std::string> DefEvent = m_DefEvents[Tokens[1]];
...
void CStatsW3MMD :: Save( CGHost *GHost, CGHostDB *DB, uint32_t GameID , std::string statstype )
{
	...
	if( DB->Begin( ) )
	{
		...
		for( map<uint32_t,std::string> :: iterator i = m_PIDToName.begin( ); i != m_PIDToName.end( ); i++ )
		{
...
vector<std::string> CStatsW3MMD :: TokenizeKey( std::string key )
{
