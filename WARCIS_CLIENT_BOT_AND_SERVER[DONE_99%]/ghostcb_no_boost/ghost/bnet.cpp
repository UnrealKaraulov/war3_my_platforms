...
void UpadeMapList( string FtpIp,
	int FtpPort,
	string FtpUser,
	string FtpPassword,
	string FtpStartDir, string mappath, string mapcfgpath )
{
	if ( AllreadyUpdate )
		return;
	...
	fs::path MapPath( mappath );
	fs::path MapCfgPath( mapcfgpath );

	//CONSOLE_Print( "Init ftp class" );
	nsFTP::CFTPClient ftpClient;
	//	CONSOLE_Print( "Init ftp connection" );
	...
	{
		//	CONSOLE_Print( "login ok!" );
			// get directory listing
		{
			nsFTP::TFTPFileStatusShPtrVec list;
			ftpClient.List( FtpStartDir, list );
			//	CONSOLE_Print( "Get files ok!" );
				// get fa
			auto LocalMapList = get_file_list( MapPath );
			vector<std::string> RemoteFileList;
			// iterate listing
				...
				it != list.end( ); ++it )
			{
				...
				string RemoveFileExt = RemoteFilePath.extension( ).string( );
				transform( RemoveFileExt.begin( ), RemoveFileExt.end( ), RemoveFileExt.begin( ), ( int( *)( int ) )tolower );

				if ( RemoveFileExt == ".w3x" || RemoveFileExt == ".w3m" )
				{
					bool needReplaceFile = true;
					fs::path FilePath = MapPath.string() + fs::u8path( ( *it )->Name( ) ).string();
					...
					if ( needReplaceFile )
					{
						...
						sw.close( );
						CONSOLE_Print( "Download " + RemoteFilePath.string( ) + " -> " + FilePath.string( ) + " -> " + to_string( ( *it )->MTime( ) ) + " -> " + to_string( localfiletime ) );
					}
				}
			}
			...
			if ( LocalMapList.size( ) && RemoteFileList.size( ) )
			{
				for ( auto locfile : LocalMapList )
				{
					...
					for ( auto remfile : RemoteFileList )
					{
						if ( locfile.find( remfile ) != string::npos )
						{
							FileFound = true;
						}
		...
		{
			nsFTP::TFTPFileStatusShPtrVec list;
			ftpClient.List( FtpStartDir + "ghost_stats_cfgs/", list );
			//	CONSOLE_Print( "Get files ok!" );
				// get fa
				...
				it != list.end( ); ++it )
			{
				...
				bool needReplaceFile = true;
				fs::path FilePath = MapCfgPath.string() + fs::u8path(  ( *it )->Name( ) ).string( );
				auto filedata = readFile( FilePath.string( ) + ".time" );
				time_t localfiletime = 0;
				time_t remotefiletime = ( *it )->MTime( );
		...
		//CONSOLE_Print( "Print files ok!" );
		ftpClient.Logout( );
		//	CONSOLE_Print( "Logout ok!" );
	}
	//else
...
bool UpdateThreadInitialized = false;
std::thread udpatethr;
...
CBNET::CBNET( CGHost *nGHost, string nServer, string nServerAlias, string nBNLSServer, uint16_t nBNLSPort, uint32_t nBNLSWardenCookie, string nCDKeyROC, string nCDKeyTFT, string nCountryAbbrev, string nCountry, uint32_t nLocaleID, string nUserName, string nUserPassword, string nFirstChannel, string nRootAdmin, string nLANRootAdmin, char nCommandTrigger, bool nHoldFriends, bool nHoldClan, bool nPublicCommands, unsigned char nWar3Version, BYTEARRAY nEXEVersion, BYTEARRAY nEXEVersionHash, string nPasswordHashType, string nPVPGNRealmName, uint32_t nMaxMessageLength, uint32_t nHostCounterID )
{
	...
	CONSOLE_Print( "[GHOST] NEW CBNET 2" );
...
vector<pair<string, int> > CBNET::GetAdmins( )
{
	...
	vector<std::string> roots = UTIL_Tokenize( m_RootAdmin, ' ' );

	for ( vector<std::string> ::iterator i = roots.begin( ); i != roots.end( ); i++ )
		result.push_back( pair<string, int>( *i, 2 ) );

	for ( vector<std::string> ::iterator i = m_Admins.begin( ); i != m_Admins.end( ); i++ )
			...
			case CBNETProtocol::SID_AUTH_ACCOUNTLOGON:
				if ( m_Protocol->RECEIVE_SID_AUTH_ACCOUNTLOGON( Packet->GetData( ) ) )
				{
					...
					if ( m_PasswordHashType == "pvpgn" )
					{
						...
						bcrypt_hashpw( m_UserPassword.c_str( ), salt, hash );

						m_Socket->PutBytes( m_Protocol->SEND_AH_PACKET( 0, 0xABCD, 0xA, 0xB, 0xC, 0xD, m_UserPassword, hash ) );
...
void CBNET::ProcessChatEvent( CIncomingChatEvent *chatEvent )
{
	...
	if ( Event == CBNETProtocol::EID_WHISPER || Event == CBNETProtocol::EID_TALK || Event == 29 )
	{
		...
		if ( Event == CBNETProtocol::EID_WHISPER && m_GHost->m_CurrentGame )
		{
			...
			else if ( Message.find( m_GHost->m_CurrentGame->GetGameName( ) ) != string::npos )
			{
				...
				if ( m_PasswordHashType == "pvpgn" && User == m_PVPGNRealmName )
				{
					...
					vector<std::string> Tokens = UTIL_Tokenize( Message, ' ' );
		...
		transform( MessageLower.begin( ), MessageLower.end( ), MessageLower.begin( ), ( int( *)( int ) )tolower );
			...
			|| MessageLower.find( "-l" ) == 0 ) )
		{
			...
			if ( IsAdmin( User ) || IsRootAdmin( User ) )
			{
				...
				******************/
				...
				if ( Command == "updatemaplist"  )
				{
					string FtpIp = string( );
					int FtpPort;
					string FtpUser = string( );
					string FtpPassword = string( );
					string FtpStartDir = string( );
					stringstream SS;
					SS << Payload;
					bool allftpdatafound = false;
					if ( !SS.eof( ) )
					{
						SS >> FtpIp;
						if ( !SS.eof( ) )
					...
					if ( allftpdatafound && IsRootAdmin( User ) )
					{
						CONSOLE_Print("[BNET: " + m_ServerAlias + "] admin [" + User + "] req update maps!");

						if ( !UpdateThreadInitialized )
						{
							UpdateThreadInitialized = true;
							udpatethr = std::thread{ UpadeMapList, FtpIp, FtpPort, FtpUser,FtpPassword ,FtpStartDir,m_GHost->m_MapPath,m_GHost->m_MapCFGPath };
						}
						else if ( UpdateThreadInitialized && !AllreadyUpdate )
						{
							if ( udpatethr.joinable( ) )
							{
								udpatethr.join( );
							}
					...
					{
						CONSOLE_Print("[BNET: " + m_ServerAlias + "] admin [" + User + "] cant update maps!");
				...
				if ( ( Command == "abort" || Command == "a" ) && m_GHost->m_CurrentGame->m_CountDownStarted && !m_GHost->m_CurrentGame->m_GameLoading && !m_GHost->m_CurrentGame->m_GameLoaded )
				{
					m_GHost->m_CurrentGame->SendAllChat( m_GHost->m_Language->CountDownAborted( ) );
					m_GHost->m_CurrentGame->m_CountDownStarted = false;
					m_GHost->m_CurrentGame->m_AutoStartPlayers = 0;
					m_GHost->m_CurrentGame->m_UsingStart = false;
				}

				if ( Command == "hcl" && !m_GHost->m_CurrentGame->m_CountDownStarted )
				{
					if ( !Payload.empty( ) )
					{
						if ( Payload.size( ) <= m_GHost->m_CurrentGame->m_Slots.size( ) )
						{
							...
							if ( Payload.find_first_not_of( HCLChars ) == string::npos )
							{
								...
								// @disturbed_oc
								m_GHost->m_CurrentGame->m_HCLOverride = true;
								// @end
							}
							else
							...
							m_GHost->m_CurrentGame->SendAllChat( m_GHost->m_Language->UnableToSetHCLTooLong( ) );
					}
					else
				...
				if ( Command == "hold" && !Payload.empty( ) && m_GHost->m_CurrentGame )
				{
					...
					m_GHost->m_CurrentGame->ClearSlots( );
				}

				if ( Command == "holdslot" && !Payload.empty( ) && m_GHost->m_CurrentGame )
				{
					stringstream SS;
					SS << Payload;
					...
					while ( !SS.eof( ) )
					{
						...
						if ( SS.fail( ) )
						{
							CONSOLE_Print( "[BNET: " + m_ServerAlias + "] bad input to holdslot command" );
							break;
						}
						else
						{
							CONSOLE_Print("[BNET: " + std::to_string(ColorID) + "] reserved");
							m_GHost->m_CurrentGame->AddReservedSlot(ColorID);
						}
					}
				}
				...
				// !HOSTSG
				//
				if (m_GHost->m_CurrentGame && Command == "corace" && !Payload.empty() && !m_GHost->m_CurrentGame->m_GameLoading && !m_GHost->m_CurrentGame->m_GameLoaded && !m_GHost->m_CurrentGame->m_SaveGame)
				{
					...
					while (!SS.fail() && !SS.eof())
					{
						...
						if (!SS.fail() && !SS.eof())
						{
							...
							unsigned char SID = m_GHost->m_CurrentGame->GetSIDFromColour(Slot);
							CONSOLE_Print("[CHANGE RACE]....");

							if (SID >= 0 && !(m_GHost->m_CurrentGame->m_Map->GetMapOptions() & MAPOPT_FIXEDPLAYERSETTINGS) && !(m_GHost->m_CurrentGame->m_Map->GetMapFlags() & MAPFLAG_RANDOMRACES) && SID < m_GHost->m_CurrentGame->m_Slots.size())
							{
								CONSOLE_Print("[CHANGE RACE : " + std::to_string(SID) + " to race " + std::to_string(Race));
								{
									if (Race == 1)
									{
										m_GHost->m_CurrentGame->m_Slots[SID].SetRace(SLOTRACE_HUMAN | SLOTRACE_SELECTABLE);
									}
									else if (Race == 2)
									{
										m_GHost->m_CurrentGame->m_Slots[SID].SetRace(SLOTRACE_ORC | SLOTRACE_SELECTABLE);
									}
									else if (Race == 4)
									{
										m_GHost->m_CurrentGame->m_Slots[SID].SetRace(SLOTRACE_NIGHTELF | SLOTRACE_SELECTABLE);
									}
									else if (Race == 3)
									{
										m_GHost->m_CurrentGame->m_Slots[SID].SetRace(SLOTRACE_UNDEAD | SLOTRACE_SELECTABLE);
									}
									else if (Race == 0)
									{
										m_GHost->m_CurrentGame->m_Slots[SID].SetRace(SLOTRACE_RANDOM | SLOTRACE_SELECTABLE);
									}

									//m_GHost->m_CurrentGame->SendAllSlotInfo();
								}
							}
						}
					}
				...
				if ( Command == "hostsg" && !Payload.empty( ) )
					m_GHost->CreateGame( m_GHost->m_Map, GAME_PRIVATE, true, Payload, User, User, m_Server, Whisper, "default" );
				...
				if ( Command == "save" )
				{
					...
					{
						CONSOLE_Print( "Error in map read!" );
					}
				}
				...
				if ( Command == "load" )
				{
					...
					else
					{
						...
						try
						{
							fs::path MapCFGPath( m_GHost->m_MapCFGPath );
							...
							else
							{
								...
								for ( fs::directory_iterator i( MapCFGPath ); i != EndIterator; i++ )
								{
				...
				if ( Command == "map" )
				{
					...
					else
					{
						...
						try
						{
							fs::path MapPath( m_GHost->m_MapPath );
							string Pattern = Payload;
							transform( Pattern.begin( ), Pattern.end( ), Pattern.begin( ), ( int( *)( int ) )tolower );
							...
							else
							{
								...
								for ( fs::directory_iterator i( MapPath ); i != EndIterator; i++ )
								{
									string FileName = i->path( ).filename( ).string( );
									string Stem = i->path( ).stem( ).string( );
									transform( FileName.begin( ), FileName.end( ), FileName.begin( ), ( int( *)( int ) )tolower );
				...
				if ( Command == "comp" && !Payload.empty( ) &&
					!m_GHost->m_CurrentGame->m_GameLoading &&
					!m_GHost->m_CurrentGame->m_GameLoaded &&
					!m_GHost->m_CurrentGame->m_SaveGame )
				{
					...
					while(!SS.eof( ))
					{
						...
						if ( SS.fail( ) )
							CONSOLE_Print( "[GAME: " + m_GHost->m_CurrentGame->m_GameName + "] bad input #1 to comp command" );
						else
						{
							if ( !SS.eof( ) )
								SS >> Skill;
							...
							if ( SS.fail( ) )
								CONSOLE_Print( "[GAME: " + m_GHost->m_CurrentGame->m_GameName + "] bad input #2 to comp command" );
							else
								m_GHost->m_CurrentGame->ComputerSlot( ( unsigned char )( Slot - 1 ), ( unsigned char )Skill, true );
						}
					}
				}
				//
				...
				if ( Command == "priv" && !Payload.empty( ) )
					m_GHost->CreateGame( m_GHost->m_Map, GAME_PRIVATE, false, Payload, User, User, m_Server, Whisper, "default" );
				...
				if ( Command == "privby" && !Payload.empty( ) )
				{
					...
					if ( GameNameStart != string::npos )
					{
						...
						m_GHost->CreateGame( m_GHost->m_Map, GAME_PRIVATE, false, GameName, Owner, User, m_Server, Whisper, "default" );
					}
				}
				...
				if ( Command == "pub" && !Payload.empty( ) )
					m_GHost->CreateGame( m_GHost->m_Map, GAME_PUBLIC, false, Payload, User, User, m_Server, Whisper, "default" );
				if ( Command == "superkick" && !Payload.empty( ) )
				{
					std::vector<std::string> kick_args = split_command( Payload, 1 );


					for ( auto game : m_GHost->m_Games )
					{
						CGamePlayer *LastMatch = NULL;
						uint32_t Matches = game->GetPlayerFromNamePartial( kick_args[ 0 ], &LastMatch );
						...
						if ( Matches == 1 )
						{
							...
							if ( !game->m_GameLoading && !game->m_GameLoaded )
								game->OpenSlot( game->GetSIDFromPID( LastMatch->GetPID( ) ), false );
						}

						game->SendAllChat( string( "Player:" ) + kick_args[ 0 ] + kick_args[ 1 ] );
					}

					if ( m_GHost->m_CurrentGame )
					{
						CGamePlayer *LastMatch = NULL;
						uint32_t Matches = m_GHost->m_CurrentGame->GetPlayerFromNamePartial( kick_args[ 0 ], &LastMatch );
						...
						if ( Matches == 1 )
						{
							...
							if ( !m_GHost->m_CurrentGame->m_GameLoading && !m_GHost->m_CurrentGame->m_GameLoaded )
								m_GHost->m_CurrentGame->OpenSlot( m_GHost->m_CurrentGame->GetSIDFromPID( LastMatch->GetPID( ) ), false );
						}

						m_GHost->m_CurrentGame->SendAllChat( string( "Player:" ) + kick_args[ 0 ] + kick_args[ 1 ] );
					}

				}
				else if ( Command == "hostfornewclient" && Payload.empty( ) && m_GHost->m_CurrentGame )
				{
					// Autostart
					// No ping
					// 
					...
					//uint32_t SID;
					//SS >> SID;
					//m_GHost->m_CurrentGame->m_UsingStart = true;
					m_GHost->m_CurrentGame->m_FastCooldown = true;
					CONSOLE_Print( "[BNET: " + m_ServerAlias + "] hostfornewclient" );
					//m_GHost->m_CurrentGame->m_AutoStartPlayers = SID;
					m_GHost->m_CurrentGame->CloseAllSlots( );
					CONSOLE_Print( "[BNET: " + m_ServerAlias + "] hostfornewclient initialized" );
				}
				else if ( Command == "host" && !Payload.empty( ) && !m_GHost->m_CurrentGame )
				{
					/*

					std::string HostCmd = fmt::format( "!host stats {} {} {} {} {}", args[ 1 ],
					...
					*/
					auto cEvent = chatEvent;
					std::vector<std::string> hostcmd = split_command( Payload, 6 );
					std::string mode = hostcmd[ 4 ];
					std::string cfgname = hostcmd[ 1 ] + "_" + hostcmd[ 2 ]/* + ".cfg"*/;

					cEvent->m_Message = "!load " + cfgname;
					ProcessChatEvent( cEvent );

					/*	int id = 0;
					for ( auto s : hostcmd )
					{
					std::cout << "Arg id:" <<  ++id << s << endl;
					}
					*/

					if ( m_GHost->m_Map && ( hostcmd[ 0 ] == "private" || hostcmd[ 0 ] == "custom" ) )
					{
						m_GHost->m_Map->SetMapType( "" );
					}

					m_GHost->CreateGame( m_GHost->m_Map, hostcmd[ 0 ] == "private" ? GAME_PRIVATE : GAME_PUBLIC, false, hostcmd[ 6 ], User, hostcmd[ 5 ], m_Server, Whisper, hostcmd[ 1 ] );

					if ( m_GHost->m_CurrentGame &&  hostcmd[ 4 ] != "default" )
					{
						m_GHost->m_CurrentGame->m_HCLCommandString = hostcmd[ 4 ];
					}
					...
					//host private code players teambalance mode name
				}
				...
				if ( Command == "pubby" && !Payload.empty( ) )
				{
					...
					if ( GameNameStart != string::npos )
					{
						Owner = Payload.substr( 0, GameNameStart );
						GameName = Payload.substr( GameNameStart + 1 );
						m_GHost->CreateGame( m_GHost->m_Map, GAME_PUBLIC, false, GameName, Owner, User, m_Server, Whisper, "default" );
					}
				}
...
void CBNET::RemoveAdmin( string name )
{
	...
	for ( vector<std::string> ::iterator i = m_Admins.begin( ); i != m_Admins.end( ); )
	{
		if ( *i == name )
			i = m_Admins.erase( i );
		else
