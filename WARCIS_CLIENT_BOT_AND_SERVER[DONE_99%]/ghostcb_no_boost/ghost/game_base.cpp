...
CBaseGame::CBaseGame( CGHost *nGHost, CMap *nMap, CSaveGame *nSaveGame, uint16_t nHostPort, unsigned char nGameState, std::string nGameName, std::string nOwnerName, std::string nCreatorName, std::string nCreatorServer )
{
	m_GHost = nGHost;
...
//here
void CBaseGame::InitAlternativeGameSlots( )
{
	m_SlotInfoChanged = true;
	if ( !m_ReservedSlots.empty( ) )
	{
		if ( m_ReservedSlots.size( ) <= m_Reserved.size( ) )
		{
			int id = 0;

			for ( auto newcolor : m_ReservedSlots )
			{
				std::string pname = m_Reserved[ id ];
				id++;

				auto p = GetPlayerFromName( pname, false );
				if ( !p )
				{
					CONSOLE_Print( "[GAME: " + m_GameName + "] error player " + pname + " not found!" );
					continue;
				}
				auto pid = p->GetPID( );
				auto sid = GetSIDFromPID( pid );

				if ( pid != 255 && sid != 255 )
				{
					//auto newsid = GetSIDFromPID( newpid + 1);
					//if ( newsid != 255 )
					//{
					if (newcolor >= 200)
						SwapSlots(sid, newcolor - 200);
					else
						SwapSlots(sid, newcolor, true);

					CONSOLE_Print( "[GAME: " + m_GameName + "] swap slot " + to_string( sid ) + " to " + to_string(newcolor) );
					//}
					//else
					//{
					//	SwapSlots(sid, newpid);
					//	CONSOLE_Print("[GAME: " + m_GameName + "] swap slot bad" + to_string(sid) + " to " + to_string(newpid));
					//}
				}
				else
				{
					CONSOLE_Print( "[GAME: " + m_GameName + "] pid or sid == 255: !" + to_string( pid ) + " " + to_string( sid ) );
				}
			}
		}
		else
		{
			CONSOLE_Print("[GAME: " + m_GameName + "] swap slot error");
...
bool CBaseGame::Update( void *fd, void *send_fd )
{
	...
	if ( m_RefreshError && !m_CountDownStarted && m_GameState == GAME_PUBLIC && !m_GHost->m_AutoHostGameName.empty( ) && m_GHost->m_AutoHostMaximumGames != 0 && m_GHost->m_AutoHostAutoStartPlayers != 0 && m_AutoStartPlayers != 0 )
	{
		...
		std::string GameName = m_GHost->m_AutoHostGameName + " #" + UTIL_ToString( m_GHost->m_HostCounter );
	...
	if ( !m_RefreshError && !m_CountDownStarted && m_GameState == GAME_PUBLIC && GetSlotsOpen( ) > 0 && GetTime( ) - m_LastRefreshTime >= 3 )
	{
		...
		for ( vector<CBNET *> ::iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
		{
			...
			if ( ( *i )->GetOutPacketsQueued( ) <= 1 )
			{
				...
				(*i)->QueueGameRefresh( m_GameState, m_GameName, std::string( ), m_Map, m_SaveGame, GetTime( ) - m_CreationTime, m_HostCounter );
				*/

				// Refresh code from Fire86 (http://codelain.com/forum/index.php?topic=11373.msg88767#msg88767)
				( *i )->QueueGameRefresh( m_GameState, m_GameName, std::string( ), m_Map, m_SaveGame, 0, m_HostCounter );
	...
	if ( m_UsingStart == true )
	{
		...
		if ( !m_CountDownStarted && m_AutoStartPlayers != 0 && (GetTime( ) - m_LastAutoStartTime >= 5 || m_FastCooldown) )
		{
		...
		if ( !m_CountDownStarted && m_AutoStartPlayers != 0 && ( GetTime( ) - m_LastAutoStartTime >= 10 || m_FastCooldown )  )
		{
			StartCountDownAuto( m_GHost->m_RequireSpoofChecks );
			m_LastAutoStartTime = GetTime( );
		}
	...
	if ( m_GHost->m_UseNormalCountDown )
	{
		...
		{
			if ( m_CountDownCounter > 0 && !m_FastCooldown )
			{
				m_CountDownCounter--;
			}
	...
	else
	{
		...
		if ( m_CountDownStarted && GetTicks( ) >= m_LastCountDownTicks + 500 )
		{
			if ( m_CountDownCounter > 0 && !m_FastCooldown )
			{
				...
				SendAllChat( ":c:FFFF0000" + UTIL_ToString( m_CountDownCounter ) + ". . ." );
				m_CountDownCounter--;
			}
			else if ( !m_GameLoading && !m_GameLoaded )
				EventGameStarted( );
	...
	if ( m_GameOverTime != 0 && GetTime( ) - m_GameOverTime >= 600 )
	{
	...
	if ( m_Players.empty( ) && ( m_GameLoading || m_GameLoaded ) )
	{
		if ( !m_Saving )
		{
			CONSOLE_Print( "[GAME: " + m_GameName + "] is over (no players left) savedata and end..." );
			SaveGameData( );
			m_Saving = true;
		}
	}

	if ( m_Saving && IsGameDataSaved( ) )
	{
		CONSOLE_Print( "[GAME: " + m_GameName + "] is over..." );
		return true;
	}
...
void CBaseGame::SendChat( unsigned char fromPID, unsigned char toPID, std::string message )
{
	//for ( vector<CBNET *> ::iterator i = m_GHost->m_BNETs.begin( ); i != m_GHost->m_BNETs.end( ); i++ )
	//{
	//	( *i )->QueueChatCommand( "/gamemessage <" + m_GameName + ">X<" + message + ">" );
	//}
	SendChat( fromPID, GetPlayerFromPID( toPID ), ":c:FF0000FF:" + message );
}
...
void CBaseGame::SendAllSlotInfo( )
{
	...
	{
		CONSOLE_Print( "[GAME: " + m_GameName + "] try to sendslotinfo in loading time " );
	}
}
...
void CBaseGame::SendAllActions( )
{
	...
	/*	m_LastActionLateBy -= m_Latency;
		...
		if ( m_LastActionLateBy > m_Latency * 5 )*/
		m_LastActionLateBy = m_Latency;
	}
...
void CBaseGame::SendWelcomeMessage( CGamePlayer *player )
{
	...
	if ( in.fail( ) )
	{
		...
		SendChat( player, " " );
		SendChat( player, " This game is hosted with Warcis Host Bot" );
		SendChat( player, "Forums:                                   https://forum.warcis.com/" );
		SendChat( player, "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-" );
		SendChat( player, " The Owner is " + m_CreatorName );
...
{
	CONSOLE_Print("[GAME: " + m_GameName + "] EventPlayerJoined ok 1");

	if (m_GHost->m_UseNormalCountDown && m_CountDownStarted)
	{
		CONSOLE_Print("[GAME: " + m_GameName + "] EventPlayerJoined error 1");

		potential->GetSocket()->PutBytes(m_Protocol->SEND_W3GS_REJECTJOIN(REJECTJOIN_FULL));
		potential->SetDeleteMe(true);
		return;
	}
	CONSOLE_Print("[GAME: " + m_GameName + "] EventPlayerJoined ok 2");
	...
	CONSOLE_Print("[GAME: " + m_GameName + "] EventPlayerJoined ok 3");
	...
	CONSOLE_Print("[GAME: " + m_GameName + "] EventPlayerJoined ok 4");
	...
	CONSOLE_Print("[GAME: " + m_GameName + "] EventPlayerJoined ok 5");
	...
	CONSOLE_Print("[GAME: " + m_GameName + "] EventPlayerJoined ok 6");
	...
	{
		int num;
		vector<std::string> ApprovedLocations;
	...
	CONSOLE_Print("[GAME: " + m_GameName + "] EventPlayerJoined ok 7");
	...
	CONSOLE_Print("[GAME: " + m_GameName + "] EventPlayerJoined ok 8");
	...
	CONSOLE_Print("[GAME: " + m_GameName + "] EventPlayerJoined ok 9");
	...
	CONSOLE_Print("[GAME: " + m_GameName + "] EventPlayerJoined ok 10");
	...
	CONSOLE_Print("[GAME: " + m_GameName + "] EventPlayerJoined ok 11");
	...
	CONSOLE_Print("[GAME: " + m_GameName + "] player [" + joinPlayer->GetName() + "|" + potential->GetExternalIPString() + "] joined the game. SID:" + std::to_string(SID));
	...
	if (m_SaveGame)
	{
		EnforceSlot.SetRace(m_Slots[SID].GetRace());
	...
	else
	{
		...
		else
		{
			if ( m_Map->GetMapFlags( ) & MAPFLAG_RANDOMRACES )
				m_Slots[ SID ] = CGameSlot( Player->GetPID( ), 255, SLOTSTATUS_OCCUPIED, 0, 12, 12, m_Slots[SID].GetRace());
			else
				m_Slots[ SID ] = CGameSlot( Player->GetPID( ), 255, SLOTSTATUS_OCCUPIED, 0, 12, 12, m_Slots[SID].GetRace());
...
void CBaseGame::EventPlayerPongToHost( CGamePlayer *player, uint32_t pong )
{
	...
	if ( !m_GameLoading && !m_GameLoaded && !player->GetDeleteMe( ) && !player->GetReserved( ) && player->GetNumPings( ) >= 1 && player->GetPing( m_GHost->m_LCPings ) > m_GHost->m_AutoKickPing )
	{
...
void CBaseGame::AutoSetHCL( )
{
	...
	std::string m_Mode = std::string( );
	std::string m_Modes = std::string( );
	std::string m_Modes2 = std::string( );
	if ( m_GHost->m_HCLCommandFromGameName )
		if ( gameName.find( "-" ) != std::string::npos )
		{
			...
			while ( j < gameName.length( ) - 1 && ( gameName.find( "-", j ) != std::string::npos ) )
			{
				k = gameName.find( "-", j );
				i = gameName.find( " ", k );
				if ( i == 0 )
			...
			if ( !m_Mode.empty( ) )
			{
				...
				if ( m_Mode.size( ) <= m_Slots.size( ) )
				{
					...
					if ( m_Mode.find_first_not_of( HCLChars ) == std::string::npos )
					{
						SetHCL( m_Mode );
...
void CBaseGame::EventGameStarted( )
{
	...
	if ( m_Map->GetMapHCLFromGameName( ) || !m_HCLOverride )
	{
		...
		vector<std::string> ValidModes = UTIL_Tokenize( m_Map->GetMapHCLValidModes( ), ' ' );
		std::string::size_type loc;

		for ( vector<std::string> ::iterator i = ValidModes.begin( ); i != ValidModes.end( ); i++ )
		{
...
unsigned char CBaseGame::GetSIDFromColour(unsigned char colour)
{
	for (unsigned char i = 0; i < m_Slots.size(); i++)
	{
		if (m_Slots[i].GetColour() == colour)
...
void CBaseGame::SwapSlots( unsigned char SID1, unsigned char SID22, bool force )
{
	unsigned char SID2 = SID22;

	if (force)
	{
		SID2 = GetSIDFromColour(SID22);
	}
	...
	if ( SID1 < m_Slots.size( ) && SID2 < m_Slots.size( ) && SID1 != SID2 )
	{
		...
		if ( Slot1.m_DefaultComputer > 0 || Slot1.m_DefaultSlotStatus > 0 )
		{
			return;
		}
		if ( Slot2.m_DefaultComputer > 0 || Slot2.m_DefaultSlotStatus > 0 )
		{
			return;
		}
...
void CBaseGame::OpenSlot( unsigned char SID, bool kick )
{
	if ( SID < m_Slots.size( ) )
	{
		...
		if ( Slot.m_DefaultComputer > 0 || Slot.m_DefaultSlotStatus > 0 )
		{
			return;
		}
...
void CBaseGame::ShuffleSlots( )
{
	...
	if ( m_Map->GetMapOptions( ) & MAPOPT_FIXEDPLAYERSETTINGS )
	{
		...
		std::shuffle(SIDs.begin(), SIDs.end(), g);
	...
	else
	{
		...
		std::shuffle(PlayerSlots.begin(), PlayerSlots.end(), g);
	}
...
void CBaseGame::AddToReserved( std::string name )
{
	...
	for ( vector<std::string> ::iterator i = m_Reserved.begin( ); i != m_Reserved.end( ); i++ )
	{
		if ( *i == name )
			return;
	}
...
bool CBaseGame::IsCreator( std::string name )
{
	std::string CreatorLower = m_CreatorName;
	transform( name.begin( ), name.end( ), name.begin( ), ( int( *)( int ) )tolower );
	transform( CreatorLower.begin( ), CreatorLower.end( ), CreatorLower.begin( ), ( int( *)( int ) )tolower );
	return name == CreatorLower;
}
