...
protected:
	...
	std::vector<int> m_ReservedSlots;
...
public:
	...
	//here
	virtual void ClearSlots() { m_ReservedSlots.clear(); }
	virtual void AddReservedSlot(int id) {
		m_ReservedSlots.push_back(id);
		m_AutoStartPlayers = m_ReservedSlots.size();

		CONSOLE_Print("Add slot to swap:" + std::to_string(id));
	}
	...
	virtual void SetHCL(std::string nHCL) { m_HCLCommandString = nHCL; }
	...
	virtual std::string GetHCL() { return m_HCLCommandString; }
	...
	//here
	virtual void InitAlternativeGameSlots();
	...
	virtual unsigned char GetSIDFromColour(unsigned char colour);
	...
	virtual void SwapSlots(unsigned char SID1, unsigned char SID2,bool force = false);
	...
	virtual bool IsCreator(std::string name);
