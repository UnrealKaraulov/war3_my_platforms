...
#define test__uuidof(ppType) __uuidof(**(ppType))
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::GetTexture(DWORD Stage, Direct3DBaseTexture8 **ppTexture)
{
	...
	*ppTexture = nullptr;
	...
	if (BaseTextureInterface != nullptr)
	{
		...
		{
			case D3DRTYPE_TEXTURE:
				BaseTextureInterface->QueryInterface(test__uuidof(&TextureInterface),(void**)&TextureInterface);
				*ppTexture = ProxyAddressLookupTable->FindAddress<Direct3DTexture8>(TextureInterface);
				break;
			case D3DRTYPE_VOLUMETEXTURE:
				BaseTextureInterface->QueryInterface(test__uuidof(&VolumeTextureInterface), (void**)&VolumeTextureInterface);
				*ppTexture = ProxyAddressLookupTable->FindAddress<Direct3DVolumeTexture8>(VolumeTextureInterface);
				break;
			case D3DRTYPE_CUBETEXTURE:
				BaseTextureInterface->QueryInterface(test__uuidof(&CubeTextureInterface), (void**)&CubeTextureInterface);
