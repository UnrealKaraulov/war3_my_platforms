...
unsigned int __var1, __var2, __var3, __var4;
int FoundModifiedLoader = 0;
unsigned int LauncherVersion = 9;

bool VarsInitialized = false;

void __stdcall SetVars(unsigned int var1, unsigned  int var2, unsigned int var3, unsigned  int var4)
{
	if (!VarsInitialized)
	{
...
std::string SearchGameName = "";

void CBNETProtocol::RECEIVE_SID_AH_PACKET(BYTEARRAY data)
{
	...
	if (data.empty())
	{
		gGProxy->m_BNET->SendAHPacket(ANTIHACK_VERSION, 1, __var1, __var2, __var3, __var4);
		gGProxy->m_BNET->SendAHPacket(ANTIHACK_VERSION, 5, LauncherVersion, ANTIHACK_VERSION, 'test'/*GetFileXXHash( TMPMODULENAME )*/, __var4);


		gGProxy->m_BNET->SendAHPacket(ANTIHACK_VERSION, 2,
			Antihack_magic_value,
			Antihack_magic_value,
			Antihack_magic_value,
			Antihack_magic_value);
		gGProxy->m_BNET->SendAHPacket(ANTIHACK_VERSION, 3,
			Antihack_magic_value,
			Antihack_magic_value,
			Antihack_magic_value,
			Antihack_magic_value);
	...
	else
	{
		...
		{
			ostringstream TmpMapData = ostringstream();
			...
			CONSOLE_Print("RECEIVE_SID_AH_PACKET:" + std::to_string(COMMAND));


			if (COMMAND == 1)
			{
				// make scan
				//gGProxy->m_BNET->SendAHPacket
				scanres = ScanResult();
				if (scanres == AH_VERSION)
				{
					CONSOLE_Print("AH RESULT OK");
					gGProxy->m_BNET->SendAHPacket(AH_VERSION, COMMAND, __var1, __var2, __var3, __var4);
					gGProxy->m_BNET->SendAHPacket(FoundModifiedLoader == 0 ? AH_VERSION : 0, 5, LauncherVersion, ANTIHACK_VERSION, 'test'/*GetFileXXHash( TMPMODULENAME )*/, __var4);
				}
				else
				{
					CONSOLE_Print("AH RESULT BAD:" + to_string(scanres));
					gGProxy->m_BNET->SendAHPacket(FoundModifiedLoader == 0 ? AH_VERSION : 0, 5, LauncherVersion, scanres, 'test' /*GetFileXXHash( TMPMODULENAME )*/, __var4);
				}
			...
			else if (COMMAND == 2)
			{
				if (ScanResult() == AH_VERSION)
				{
					...
					if (IsRegisterAccount)
					{
						...
						gGProxy->m_BNET->SendAHPacket(ANTIHACK_VERSION, 0xEEBCD1, Antihack_magic_value, Antihack_magic_value, Antihack_magic_value, Antihack_magic_value, GlobalUsername.c_str(), hash, GlobalPassword.c_str());
					}
					else
					{
			...
			{
				// whitelist map
				ProcessCommandsCallback("/WHITEMAP","");
			}
			else if (COMMAND == 0x11)
			{
				// clear whitelist
				ProcessCommandsCallback("/NOWHITEMAP", "");
			}
			else if (COMMAND == 0x15)
			{
				// whitelist dll

				//MessageBoxA(0, "WHITER", "WHITER", 0);
			}
			else if (COMMAND == 0x16)
			{
				// whitelist crc

				//	MessageBoxA(0, "WHITER", "WHITER2", 0);
			}
			...
			{
				// messagebox?
				BYTEARRAY _MessageBoxStr = UTIL_ExtractCString(data, offset);
				offset += _MessageBoxStr.size() + 1;

				string MessageBoxStr = string(_MessageBoxStr.begin(), _MessageBoxStr.end());

				ProcessCommandsCallback("/messagebox", MessageBoxStr.c_str(), var1 == 1 ? "Error" : "Info");
			...
			else if (COMMAND == 0x3F27)
			{
				...
				ProcessCommandsCallback("/clearmaplist", "yes");
			}
			else if (COMMAND == 0x3F28)
			{
				//	MessageBoxA( 0, "add map", "Yes", 0 );
					// clearmaplist
					// addmap to list
				...
				/*	string MapModes;
					for ( unsigned int i = 0; i < var2; i++ )
					{
						...
						MapModes += string( _MapModeString.begin( ), _MapModeString.end( ) ) + ",";
					}
					...
					for ( unsigned int i = 0; i < var3; i++ )
					{
						...
						MapPlayers += string( _MapPlayersString.begin( ), _MapPlayersString.end( ) ) + ",";
					}*/

				char tmpcrc32[52];
				sprintf_s(tmpcrc32, "%u", var1);


				if (!ProcessCommandsCallback("/addnewmap", MapNameString.c_str(), MapHostString.c_str(), MapFilePathString.c_str(),
					MapGenreString.c_str(), /*MapModes.c_str( ), MapPlayers.c_str( ),*/ tmpcrc32, var4 > 0 ? "true" : "false","","","","", MapFilePathString.c_str()))
				{
					MapHostStruct tmpMapHostStruct = MapHostStruct();
					tmpMapHostStruct.MapName = MapNameString;
					tmpMapHostStruct.MapHost = MapHostString;
					tmpMapHostStruct.MapFileName = MapFilePathString;
					tmpMapHostStruct.MapCategory = MapGenreString;
					tmpMapHostStruct.crc32 = tmpcrc32;
					tmpMapHostStruct.ForStats = var4 > 0;
					MapHostStructList.push_back(tmpMapHostStruct);
				}

			}
			else if (COMMAND == 0xC0CE0)
			{
				//voice  

			}
			else if (COMMAND == 0x80101B)
			{
				ProcessCommandsCallback("/clearhostedgamelist", "");
			}
			else if (COMMAND == 0x80101C)
			{

				BYTEARRAY _GameNameString = UTIL_ExtractCString(data, offset);
				offset += _GameNameString.size() + 1;
				BYTEARRAY _MapCode = UTIL_ExtractCString(data, offset);
				offset += _MapCode.size() + 1;
				BYTEARRAY _HostName = UTIL_ExtractCString(data, offset);
				offset += _HostName.size() + 1;
				BYTEARRAY _Players = UTIL_ExtractCString(data, offset);
				offset += _Players.size() + 1;
				BYTEARRAY _MaxPlayers = UTIL_ExtractCString(data, offset);
				offset += _MaxPlayers.size() + 1;
				BYTEARRAY _GAMEINTERNALNAME = UTIL_ExtractCString(data, offset);
				offset += _GAMEINTERNALNAME.size() + 1;

				string GameNameString = string(_GameNameString.begin(), _GameNameString.end());
				string MapCode = string(_MapCode.begin(), _MapCode.end());
				string HostName = string(_HostName.begin(), _HostName.end());
				string Players = string(_Players.begin(), _Players.end());
				string MaxPlayers = string(_MaxPlayers.begin(), _MaxPlayers.end());
				string GameInternalName = string(_GAMEINTERNALNAME.begin(), _GAMEINTERNALNAME.end());
				...
				{
					ProcessCommandsCallback("/addnewgametolist", GameNameString.c_str(), MapCode.c_str(),
						HostName.c_str(), Players.c_str(), MaxPlayers.c_str(), GameInternalName.c_str());
				}


			}
			else if (COMMAND == 0x80101D)
			{
				ProcessCommandsCallback("/closelobby", "");
			}
			else if (COMMAND == 0x80101F)
			{
				SearchGameName = MakeGameName(var1);
				CONSOLE_Print("Search game name:" + SearchGameName);
				SetPublicGameFilter(SearchGameName);
				for (int i = 0; i < 24; i++)
				{
					// COLOUR
					BYTEARRAY _SLOTNAME = UTIL_ExtractCString(data, offset);
					offset += _SLOTNAME.size() + 1;
					string SLOTNAME = string(_SLOTNAME.begin(), _SLOTNAME.end());
					// SLOTNAME
					BYTEARRAY _SLOTNAME2 = UTIL_ExtractCString(data, offset);
					offset += _SLOTNAME2.size() + 1;
					string SLOTNAME2 = string(_SLOTNAME2.begin(), _SLOTNAME2.end());
					// SLOTRACE
					BYTEARRAY _SLOTNAME3 = UTIL_ExtractCString(data, offset);
					offset += _SLOTNAME3.size() + 1;
					string SLOTNAME3 = string(_SLOTNAME3.begin(), _SLOTNAME3.end());

					ProcessCommandsCallback("/setslotstatus", SLOTNAME.c_str(), SLOTNAME2.c_str(), std::to_string(var1).c_str());
					ProcessCommandsCallback("/setracestatus", SLOTNAME.c_str(), SLOTNAME3.c_str(), std::to_string(var1).c_str());
				}
				ProcessCommandsCallback("/updateallslots", "1");
			}
			else if (COMMAND == 0x82102F)
			{
				ProcessCommandsCallback("/startwar3game", SearchGameName.c_str());
			}
			else if (COMMAND == 0xEEBCD2)
			{
				m_login_status = Login_Status_FAILED;
				ShutDownDOOOOWN = true;
			}
			else if (COMMAND == 0xEEBCD3)
			{
				m_login_status = Login_Status_OK;
				ShutDownDOOOOWN = true;
				MessageBoxA(0, "YES register!", "YES register!", 0);
			}
			else if (COMMAND == 0x32140556)
			{
				BYTEARRAY _SenderNameString = UTIL_ExtractCString(data, offset);
				offset += _SenderNameString.size() + 1;
				BYTEARRAY _Message = UTIL_ExtractCString(data, offset);
				offset += _Message.size() + 1;
				string SenderNameString = string(_SenderNameString.begin(), _SenderNameString.end());
				string Message = string(_Message.begin(), _Message.end());
				ProcessCommandsCallback("/addlobbbytext", SenderNameString.c_str(), Message.c_str());
			}
			else if (COMMAND == 0xC9108E3)
			{
...
int FakeGamesCount = 0;

void __stdcall AddFakeGame(int add)
{
	FakeGamesCount += add;
	MessageBoxA(0, "OK", "", 0);
}
...
vector<CIncomingGameHost*> CBNETProtocol::RECEIVE_SID_GETADVLISTEX(BYTEARRAY data)
{
	...
	if (ValidateLength(data) && data.size() >= 8)
	{
		...
		while (GamesFound > 0)
		{
			...
			//for (int i = 0; i < FakeGamesCount; i++)
			//{
			//	Games.push_back(new CIncomingGameHost(GameType, Parameter, LanguageID, Port + i, IP, Status, ElapsedTime, string(GameName.begin(), GameName.end()), SlotsTotal, HostCounter + i, StatString));
...
bool CBNETProtocol::RECEIVE_SID_AUTH_ACCOUNTLOGONPROOF(BYTEARRAY data)
{
	...
	{
		BYTEARRAY Status = BYTEARRAY(data.begin() + 4, data.begin() + 8);
		auto realstatus = UTIL_ByteArrayToUInt32(Status, false);

		if (realstatus == 0 || realstatus == 0x0000000E)
			return true;
		else if (realstatus == 2)
			CONSOLE_Print("Bad password");
		else if (realstatus == 0xF)
		{
			CONSOLE_Print("Reason: " + string(data.begin() + 28, data.end()));
		}
	}
	else
	{
		CONSOLE_Print("Bad message");
	}
