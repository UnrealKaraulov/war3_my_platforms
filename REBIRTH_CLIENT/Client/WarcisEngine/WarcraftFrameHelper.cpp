...
namespace NWar3Frame
{
	...
	int(__thiscall* Wc3SimulateClickEventListBox)(int listboxaddr, int specialgameaddr) = NULL;
...
//}
	...
	{
		CStatusDefaultCStatus = (DWORD)GameDll + 0xA8C804;
		CStatusLoadFramesVar1 = (DWORD)GameDll + 0xACD214;
		CStatusLoadFramesVar2 = (DWORD)GameDll + 0xACD264;
		GetFrameItemAddress = (pGetFrameItemAddress)((DWORD)GameDll + 0x5FA970);
		FrameEventHandler = (int(__thiscall*)(int frameaddr, unsigned int eventid))((DWORD)GameDll + 0x62A580);
		LoadFrameDefFile = (int(__fastcall*)(const char* filename, int var1, int var2, int cstatus)) ((DWORD)GameDll + 0x5D8DE0);
		CreateNewCFrame = (int(__fastcall*) (const char* FrameName, int rframeaddr, int UnkIdx, int UnkValue, int id))((DWORD)GameDll + 0x5C9560);
		SimulateFrameEvent = (int(__thiscall*)(int FrameAddr, CFrameEventStruct * EventId))((DWORD)GameDll + 0x370710);
		SetAbsolutePosition = (void(__thiscall*)(int FrameAddr180, CFramePosition orginPosition, float absoluteX, float absoluteY, unsigned int flag))((DWORD)GameDll + 0x6061B0);
		SetRelativePosition = (void(__thiscall*)(int SrcFrameAddr, CFramePosition orginPosition, int DstFrameAddr, CFramePosition toPosition, float relativeX, float relativeY, unsigned int flag))((DWORD)GameDll + 0x606770);
		UpdatePosition = (unsigned int(__thiscall*)(int FrameAddr /* or offset 180*/, unsigned int flag))((DWORD)GameDll + 0x605CC0);
		UpdateFlags = (unsigned int(__thiscall*)(int FrameAddr, unsigned int flag))((DWORD)GameDll + 0x602370);
		DestroyCFrame = (unsigned int(__thiscall*)(int FrameAddr, unsigned int flag))((DWORD)GameDll + 0x606910);
		SetFrameTexture = (unsigned int(__thiscall*)(int FrameAddr, const char* texturepath, unsigned char flags/*border?*/, BOOL tiled, const char* borderpath, BOOL flag))((DWORD)GameDll + 0x6212D0);
		GetFrameItemAddr = (int(__fastcall*)(const char* name, int id))((DWORD)GameDll + 0x5FA970);
		Wc3ChangeMenu = (void(__fastcall*)(int, int))((DWORD)GameDll + 0x5934F0);
		TextFrameSetText = (int(__thiscall*)(int FrameAddr, const char* text))((DWORD)GameDll + 0x611D40);
		SoundPlay = (void(__fastcall*)(int, const char* name, int, int, int, int, int, int))((DWORD)GameDll + 0x32F310);
		EditboxSetMaxLen = (int(__thiscall*   /* sub_6F616250*/)(int FrameAddr, unsigned int maxlen))((DWORD)GameDll + 0x616250);
		EditBoxFrameSetText = (int(__thiscall*)(int frameaddr, const char* newtext, BOOL unk))((DWORD)GameDll + 0x615B50);
		//settextaddr =
		PopupMenuAddItem = (int(__thiscall* /*sub_6F613260*/)(int FrameAddr, const char* a2, int flag))((DWORD)GameDll + 0x613260); //flag = -2
		ClearPopupMenu = (int(__thiscall*)(int FrameAddr))((DWORD)GameDll + 0x334CF0);
		//	FixFrameHookError1 = ( void( __stdcall * )( int a1 ) )( (DWORD)GameDll + 0x5F83F0 );
		CFrameSetFocus = (int(__thiscall*)(int FrameAddr, int flag))((DWORD)GameDll + 0x336C20);
		Wc3GetSkinItemPath = (const char* (__fastcall*)(const char* name, const char* theme))((DWORD)GameDll + 0x31F530);
		GlobalGlueObj = (CGlueMgr**)((DWORD)GameDll + 0xACE66C);
		//pGameGlobalUIaddr = ( (DWORD)GameDll + 0xAB4F80 );
		GetCursorAddr = (int(__thiscall*)(int num))((DWORD)GameDll + 0x4D9830);
		Wc3SimulateClickEvent = (int(__thiscall*)(int btnaddr, int unk)) ((DWORD)GameDll + 0x601F20);
		StartSpriteModelAnimated = (int(__fastcall*)(int spritemodel, int unknown_or_anim_id, void(__fastcall * SetAnimOffset)(int real_frame, float* a2, float unk_val), int real_frame, float unk_or_start_off))((DWORD)GameDll + 0x4E8720);
		UpdateFrameScale = (void(__thiscall*)(int FrameAddr))((DWORD)GameDll + 0x605700);
		SetFrameWidth = (void(__thiscall* /*sub_6F605D90*/)(int FrameAddr, float x))((DWORD)GameDll + 0x605DB0);
		SetFrameHeight = (void(__thiscall* /*sub_6F605DB0*/)(int FrameAddr, float y)) ((DWORD)GameDll + 0x605D90);
		SetTextFrameFont = (int(__thiscall*)(int FrameAddr, const char* font, float scale, int flag))((DWORD)GameDll + 0x5FB960);
		CurrentFrameFocusedAddr = (DWORD)GameDll + 0xACE67C;
		Wc3SimulateClickEventListBox = (int(__thiscall*)(int btnaddr, int unk))((DWORD)GameDll + 0x564ED0);


		Wc3ControlClickButton_org = (pWc3ControlClickButton)((DWORD)GameDll + 0x601F20);//1.27a 0xBE3A0
...
*/
	...
	void CWar3Frame::SetFrameFocused()
	{
		...
		if (FrameType != CFrameType::FRAMETYPE_FRAME && FrameType != CFrameType::FRAMETYPE_LISTBOX)
		{
			return;
		}
	...
	int CWar3Frame::Click(int specialarg )
	{
		if (!FrameOk)
		{
			MessageBoxA(0, "Can't click on bad frame:(", 0, 0);
			return 0;
		}
		if (FrameType == CFrameType::FRAMETYPE_LISTBOX)
		{
			return Wc3SimulateClickEventListBox(FrameAddr, specialarg);
		}
		return Wc3SimulateClickEvent(FrameAddr, 1);
	}
	...
	void CWar3Frame::StopAnimate()
	{
		if (!FrameOk)
			return;
		if (FrameType == CFrameType::FRAMETYPE_SPRITE
			|| FrameType == CFrameType::FRAMETYPE_MODEL || FrameType == CFrameType::FRAMETYPE_FRAME)
		{
			if (*(int*)(FrameAddr + 372))
			{
				int SpriteMdlAddr = **(int**)(FrameAddr + 376);
	...
	bool CWar3Frame::CheckIsOk()
	{
		...
		FrameAddr = GetFrameItem(FrameName.c_str(), FrameId);
		FrameOk = true;
		return true;
	}
	...
	CWar3Frame::~CWar3Frame()
	{
		if (CurrentFrameFocusedAddr && *(int*)CurrentFrameFocusedAddr == FrameAddr)
		{
			*(int*)CurrentFrameFocusedAddr = 0;
		}
		anim_offset = 0.0f;
	...
	//	CWar3Frame::Init( 0, (DWORD)GameDll );
