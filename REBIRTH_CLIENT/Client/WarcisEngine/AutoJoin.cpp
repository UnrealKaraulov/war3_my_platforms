...
DWORD WINAPI DestroyНовыйПотокДляАвтоматическогоВхода(LPVOID)
{
	Sleep(100);
	Доступ_К_WarcraftEngine();
...
DWORD WINAPI НовыйПотокДляАвтоматическогоВхода(LPVOID)
{
	...
	*(int*)((DWORD)GameDll + 0xA9E7A4) = 1;
	DestroyAutoJoinThreadHandle = CreateThread(0, 0, DestroyНовыйПотокДляАвтоматическогоВхода, 0, 0, 0);
