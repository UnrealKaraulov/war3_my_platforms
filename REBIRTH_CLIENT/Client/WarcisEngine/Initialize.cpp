...
typedef HMODULE(WINAPI* GetModuleHandleA_)(LPCSTR modulename);
GetModuleHandleA_ GetModuleHandleA_org;
GetModuleHandleA_ GetModuleHandleA_ptr;
...
HMODULE WINAPI GetModuleHandleA_my(LPCSTR modulename)
{
	if (!modulename || modulename[0] == '\0')
	{ 
		return GetModuleHandleA_ptr(modulename);
	}

	std::string fullpath = str_tolower(modulename);
	if (fullpath.size() > 0)
	{
		if (fullpath.find("game.dll") != std::string::npos)
		{
			return (HMODULE)(DWORD)GameDll;
		}
	}
...
bool whitewhite = false;
int __stdcall WHITEMAP(int val)
{
	if (val == 5000)
		whitewhite = true;
	else
...
BOOL __stdcall DllMain(HINSTANCE Module, unsigned int reason, LPVOID)
{
	if (reason == DLL_THREAD_ATTACH)
	{
		...
		if (!whitewhite)
		{
			if (LoadedMssPlugins >= 0 && !LASTINITIALIZED && ThreadModule != mss32mdl && ThreadModule != mswsockmdl && ThreadModule != ntdllmdl)
			{
			...
			/*else if (LoadedMssPlugins >= 0 && LASTINITIALIZED2 && ThreadModule != mss32mdl && ThreadModule != mswsockmdl && ThreadModule != ntdllmdl)
			{
	...
	if (reason == DLL_PROCESS_ATTACH)
	{
		...
		GetModuleHandleA_org = (GetModuleHandleA_)GetProcAddress_ptr(
			GetModuleHandle(TEXT("kernel32.dll")),
			"GetModuleHandleA");
		if (!GetModuleHandleA_org)
		{
			GetModuleHandleA_org = GetModuleHandleA;
		}
		MH_CreateHook(GetModuleHandleA_org, &GetModuleHandleA_my, reinterpret_cast<void**>(&GetModuleHandleA_ptr));
		MH_EnableHook(GetModuleHandleA_org);
