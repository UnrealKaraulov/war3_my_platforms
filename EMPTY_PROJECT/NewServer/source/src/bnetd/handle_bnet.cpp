...
namespace pvpgn
{
	...
	namespace bnetd
	{
		...
		static int client_message_send(t_connection* target, SmallBinReader message);
		...
		static int _client_wc3_client(t_connection* c, t_packet const* const packet);
		...
		void client_message_create_account(t_connection* c, SmallBinReader* tmpSmallBinReader)
		{
			t_hash       passhash;
			t_account* temp;
			std::string username = tmpSmallBinReader->ReadStr();
			std::string password = tmpSmallBinReader->ReadStr();
			std::string email = tmpSmallBinReader->ReadStr();
			eventlog(eventlog_level_error, __FUNCTION__, "[{}] YEEEES!!!! Register account: {}  ---- {} --- {}", conn_get_socket(c), username, password, email);

			if (account_check_name(username.c_str()) < 0) {
				message_send_text(c, message_type_error, c, localize(c, "Account name contains some invalid symbol!"));
				return;
			}

			if (password.length() > MAX_USERPASS_LEN)
			{
				message_send_text(c, message_type_error, c, "Maximum password length");
				return;
			}
			/*	for ( int i = 0; i < password.length( ); i++ )
					password[ i ] = safe_tolower( password[ i ] );*/

			bnet_hash(&passhash, password.length(), password.c_str());


			temp = accountlist_create_account(username.c_str(), hash_get_str(passhash));
			if (!temp) {
				message_send_text(c, message_type_error, c, localize(c, "Failed to create account!"));
				eventlog(eventlog_level_debug, __FUNCTION__, "[{}] account \"{}\" not created (failed)", conn_get_socket(c), username.c_str());
				return;
			}

			account_set_email(temp, email.c_str());

			message_send_text(c, message_type_info, c, localize(c, "Account [{}] created.", account_get_uid(temp)));
			eventlog(eventlog_level_debug, __FUNCTION__, "[{}] account \"{}\" created", conn_get_socket(c), username.c_str());
		}
		...
		struct ClientConnectionStruct
		{
			...
			std::vector<t_connection*> MuteConnections;
		};

		struct ClientGameStruct
		{
			t_connection* host;
			int MaxPlayers;
			std::string GameName;
			std::vector<ClientConnectionStruct> ListConnections;
		};

		std::vector< ClientGameStruct > ClientGames;


		// Создание игры 
		// Удаление игры при выходе хоста или же при нажатии кнопки отмена
		// Перемещение между командами
		// 

		void RemoveClientGame(t_connection* conn, bool gamestarted = false)
		{
			for (unsigned int i = 0; i < ClientGames.size(); i++)
			{
				if (ClientGames[i].host == conn)
				{
					// send to all clients - end of game and remove it.
					for (auto& conn : ClientGames[i].ListConnections)
					{
						// close game with reason gamestarted
					}
		...
		void RemoveConnectionFromAllGames(t_connection* conn)
		{
			// unhost or remove players 

			RemoveClientGame(conn);

			for (auto& game : ClientGames)
			{
				bool foundconn = false;
				for (unsigned int i = 0; i < game.ListConnections.size(); i++)
				{
					if (game.ListConnections[i].conn == conn)
					{
						foundconn = true;
						game.ListConnections.erase(game.ListConnections.begin() + i);
						break;
					}
				}
				if (foundconn)
		...
		void RemoveAllPlayersFromClientGame(ClientGameStruct clientGame)
		{

		}

		void DestroyGame(std::string GameName)
		{
			for (unsigned int i = 0; i < ClientGames.size(); i++)
			{
				if (_stricmp(GameName.c_str(), ClientGames[i].GameName.c_str()) == 0)
				{
					// send to all clients - end of game and remove it.
					for (auto& conn : ClientGames[i].ListConnections)
					{
		...
		bool IsGameExist(std::string GameName)
		{
			for (unsigned int i = 0; i < ClientGames.size(); i++)
			{
				if (_stricmp(GameName.c_str(), ClientGames[i].GameName.c_str()) == 0)
		...
		int FindGameId(std::string GameName)
		{
			for (unsigned int i = 0; i < ClientGames.size(); i++)
			{
				if (ClientGames[i].ListConnections.size() < ClientGames[i].MaxPlayers)
				{
					if (_stricmp(GameName.c_str(), ClientGames[i].GameName.c_str()) == 0)
					{
						return i;
		...
		int FindEmptySlotInClientGame(ClientGameStruct& game)
		{
			for (int i = 0; i < game.MaxPlayers; i++)
			{
				bool found = false;
				for (auto& player : game.ListConnections)
				{
					if (player.slot == i)
					{
						found = true;
		...
		void AddPlayerToClientGame(t_connection* conn, ClientGameStruct& game)
		{
			ClientConnectionStruct tmpClientConnectionStruct = ClientConnectionStruct();
			tmpClientConnectionStruct.conn = conn;
			tmpClientConnectionStruct.mutespeak = false;
			tmpClientConnectionStruct.slot = FindEmptySlotInClientGame(game);
			game.ListConnections.push_back(tmpClientConnectionStruct);
		}


		void AddNewClientGame(t_connection* host, std::string GameName, int MaxPlayers)
		{
			// search and unhost old games

			RemoveConnectionFromAllGames(host);
			...
			ClientGameStruct tmpClientGameStruct = ClientGameStruct();
			tmpClientGameStruct.host = host;
			tmpClientGameStruct.GameName = GameName;
			tmpClientGameStruct.MaxPlayers = MaxPlayers;

			ClientConnectionStruct tmpClientConnectionStruct = ClientConnectionStruct();
			tmpClientConnectionStruct.conn = host;
			tmpClientConnectionStruct.mutespeak = false;
			tmpClientConnectionStruct.slot = 0;

			tmpClientGameStruct.ListConnections.push_back(tmpClientConnectionStruct);

			ClientGames.push_back(tmpClientGameStruct);

			// send create

		}

		void client_message_create_game(t_connection* c, SmallBinReader* tmpSmallBinReader)
		{
			std::string GameName = tmpSmallBinReader->ReadStr();
			int MaxPlayers = tmpSmallBinReader->ReadInt();

			if (!tmpSmallBinReader->ErrorFound)
			{
				AddNewClientGame(c, GameName, MaxPlayers);
			}
			else
			{
				// Send error
		...
		void client_message_unhost_game(t_connection* c, SmallBinReader* tmpSmallBinReader)
		{
			int flag = tmpSmallBinReader->ReadInt();

			RemoveConnectionFromAllGames(c);
		}

		void client_message_join_game(t_connection* c, SmallBinReader* tmpSmallBinReader)
		{
			RemoveConnectionFromAllGames(c);
			...
			if (!tmpSmallBinReader->ErrorFound)
			{
				int NeededGameID = FindGameId(GameName);
				if (NeededGameID > -1)
				{
					// Add to game and send message
				}
			}
		}


		void client_message_game_list(t_connection* c, SmallBinReader* tmpSmallBinReader)
		{
			int players = tmpSmallBinReader->ReadInt();
		...
		static int client_message_send(t_connection* target, SmallBinReader message)
		{
			...
			packet_set_size(rpacket, sizeof(t_client_wc3_client));
			packet_set_type(rpacket, WC3_CLIENT);

			packet_append_data(rpacket, message.ReadData().data(), message.ReadData().size());

			/* Send news packet */
			conn_push_outqueue(target, rpacket);
			packet_del_ref(rpacket);
		...
		static int client_message_send(t_connection* target, std::string message, int messagetype)
		{
			t_packet* rpacket;
			...
			packet_append_data(rpacket, &messagetype, 4);
			/* Append news to packet, we used the already cached len in the lstr */
			packet_append_string(rpacket, message.c_str());
...
#define CLIENT_MESSAGE_CREATE_ACCOUNT 0
#define CLIENT_MESSAGE_CREATE_GAME 1
#define CLIENT_MESSAGE_LEAVE_GAME 2
#define CLIENT_MESSAGE_JOIN_GAME 3
#define CLIENT_MESSAGE_SWAP_GAME_SLOT 4
#define CLIENT_MESSAGE_GAME_TEXT 5
#define CLIENT_MESSAGE_GAME_VOICE 6
#define CLIENT_MESSAGE_GAME_LIST 7
#define CLIENT_MESSAGE_GAME_ADD_SLOT 8

		// new тут обработка пакетов от клиента
		static int _client_wc3_client(t_connection* c, t_packet const* const packet)
		{
			if (packet_get_size(packet) < sizeof(t_client_wc3_client)) {
				eventlog(eventlog_level_error, __FUNCTION__, "[{}] got bad WC3_CLIENT packet (expected {} bytes, got {})", conn_get_socket(c), sizeof(t_client_wc3_client), packet_get_size(packet));
				return -1;
			}
			
			const unsigned char* data;
			if (!(data = (const unsigned char*)packet_get_raw_data_const(packet, sizeof(t_client_wc3_client)))) {
				eventlog(eventlog_level_error, __FUNCTION__, "[{}] got bad WC3_CLIENT packet (missing or too long data)", conn_get_socket(c));
				return -1;
			}

			SmallBinReader* tmpSmallBinReader = new SmallBinReader(data, (packet_get_size(packet) - sizeof(t_client_wc3_client)));

			unsigned int MessageID = tmpSmallBinReader->ReadUInt();
			if (!tmpSmallBinReader->ErrorFound)
			{
				switch (MessageID)
				{
				case CLIENT_MESSAGE_CREATE_ACCOUNT:
					client_message_create_account(c, tmpSmallBinReader);
					break;
				case CLIENT_MESSAGE_CREATE_GAME:
					client_message_create_game(c, tmpSmallBinReader);
					break;
				case CLIENT_MESSAGE_LEAVE_GAME:
					client_message_unhost_game(c, tmpSmallBinReader);
					break;
				case CLIENT_MESSAGE_JOIN_GAME:
					client_message_join_game(c, tmpSmallBinReader);
					break;
				case CLIENT_MESSAGE_GAME_LIST:
					client_message_game_list(c, tmpSmallBinReader);
					break;
				default:
					break;
				}
