...
vector<CIncomingGameHost *> CBNETProtocol :: RECEIVE_SID_GETADVLISTEX( BYTEARRAY data )
{
	...
	if( ValidateLength( data ) && data.size( ) >= 8 )
	{
		...
		while( GamesFound > 0 )
		{
			...
			/*string a(reinterpret_cast<char*>(GameType), sizeof(GameType));
			string b(reinterpret_cast<char*>(Parameter), sizeof(Parameter));
			string g(reinterpret_cast<char*>(LanguageID), sizeof(LanguageID));
			string d(reinterpret_cast<char*>(Port), sizeof(Port));
			string e(reinterpret_cast<char*>(ElapsedTime), sizeof(ElapsedTime));
			string f(reinterpret_cast<char*>(HostCounter), sizeof(HostCounter));

			CONSOLE_Print(a + " " + b +  " " +  g +" "  + d + " " + e + " " + f + " ");*/
...
{
	CONSOLE_Print( "RECEIVED SID_AUTH_ACCOUNTLOGONPROOF" );
	CONSOLE_Print( data );
	...
	if( ValidateLength( data ) && data.size( ) >= 8 )
	{
		...
		if(UTIL_ByteArrayToUInt32(Status, false) == 0 || UTIL_ByteArrayToUInt32(Status, false) == 0xE)
			return true;
	}
	else
	{
...
BYTEARRAY CBNETProtocol::SEND_SID_WC3_CLIENT( BYTEARRAY data )
{
	unsigned char Cookie[ ] = { 0, 0, 0, 0 };
...
// new
bool CBNETProtocol::RECEIVE_SID_WC3_CLIENT( BYTEARRAY data )
{
	// DEBUG_Print( "RECEIVED SID_NULL" );
	// DEBUG_Print( data );
	...
	SmallBinReader tmpSmallBinReader = SmallBinReader( data.data( ), data.size( ) );
	
	int packettype = tmpSmallBinReader.ReadInt( );
