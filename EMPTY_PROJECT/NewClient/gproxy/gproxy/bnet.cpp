...
bool CBNET::Update( void *fd, void *send_fd )
{
	...
	if ( m_Socket->GetConnecting( ) )
	{
		...
		if ( m_Socket->CheckConnect( ) )
		{
			...
			if ( m_UserName != "xxxxxxxxxxxxxxxxxxxxxxxxxx" )
			{
				CONSOLE_Print( "[BNET] connected!" );
...
void CBNET::ProcessPackets( )
{
	...
	while ( !m_Packets.empty( ) )
	{
		...
		if ( Packet->GetPacketType( ) == BNET_HEADER_CONSTANT )
		{
			switch ( Packet->GetID( ) )
			{
			case CBNETProtocol::SID_WC3_CLIENT:
				// warning: we do not respond to NULL packets with a NULL packet of our own
				// this is because PVPGN servers are programmed to respond to NULL packets so it will create a vicious cycle of useless traffic
				// official battle.net servers do not respond to NULL packets

				m_Protocol->RECEIVE_SID_WC3_CLIENT( Packet->GetData( ) );
				break;
			...
			case CBNETProtocol::SID_GETADVLISTEX:
				...
				for ( vector<CIncomingGameHost *> ::iterator i = Games.begin( ); i != Games.end( ); )
				{
					...
					//if(!gameNameInHosts((*i)->GetHostName()))
					//{
						//delete *i;
							//i = Games.erase( i );
							//continue;
...
void CBNET::ProcessChatEvent( CIncomingChatEvent *chatEvent )
{
	...
	CONSOLE_Print( "[TEST] <" + User + "> " + Message, false );
...
void CBNET::QueueChatCommand( string chatCommand, bool hidden )
{
	...
	if ( m_LoggedIn )
	{
		...
		{
			CONSOLE_Print( "[QUEUED] " + chatCommand + ( hidden ? " hidden" : " showed" ) );
			ProcessCommandsCallback( "/addlocalusetext", chatCommand.c_str( ) );
