...
namespace pvpgn
{
	namespace bnetd
	{
		...
		void CreateAlternativeGame(t_connection* host, std::string GameName, std::string MapCode, unsigned int slots, unsigned int bots)
		{
			...
			if (GameName.length() < 3 || GameName.length() > 255)
			{
			...
			SendGameToAllPlayers(SuperGameStructList[SuperGameStructList.size() - 1]);
		}
		...
		void UpdateSlot(t_connection* host, int colorid, int slotid, int team)
		{
		...
		void ChangeSlot(t_connection* host, int colorid, int slotstatus)
		{
	...
	*/
		...
		*/
		...
		extern void conn_destroy(t_connection* c, t_elem** elem, int conn_or_dead_list)
		{
			...
			//eventlog(eventlog_level_error, __FUNCTION__, "1");
			...
			eventlog(eventlog_level_error, __FUNCTION__, "3");
			conn_set_game(c, NULL, NULL, NULL, game_type_none, 0);
			...
			eventlog(eventlog_level_error, __FUNCTION__, "4");
			if (c->protocol.account)
			{
				account_set_playmap(c->protocol.account, "");
...
*/
		...
		extern int conn_push_outqueue(t_connection* c, t_packet* packet)
		{
			...
			if (prefs_get_packet_limit() && queue_get_length((t_queue const* const*)q) > prefs_get_packet_limit() && !c->protocol.IsNewClient && !c->protocol.IamBOT)
			{
				queue_clear(q);
				conn_set_state(c, conn_state_destroy);
	...
	*/
		...
		{
			eventlog(eventlog_level_trace, __FUNCTION__, "[{}] AH PACKET RECIEVED: {} ", conn_get_socket(c), command);
